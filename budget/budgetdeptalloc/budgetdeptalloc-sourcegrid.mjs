...
import * as hnd from  './budgetdeptalloc-sourcegrid-hnd.mjs'

const tbl_list = $('#pnl_editsourcegrid-tbl_list');
const txt_title = $('#pnl_editsourcegrid-title');
const pnl_control = $('.pnl_editsourcegrid-control');
const btn_removechecked  = $('#pnl_editsourcegrid-removechecked');
const btn_addrow = $('#pnl_editsourcegrid-addrow');
...
export function createnew(data, options) {
	...
	txt_title.html(data.budgetdeptalloc_descr)
...
export function OpenDetil(data) {
	...
	var fn_listloading = async (options) => {
		options.api = `${global.modulefullname}/source-list`
		options.criteria['id'] = data.budgetdeptalloc_id
	...
	var fn_listloaded = async (result, options) => {
		...
		var detilform = $ui.getPages().ITEMS['pnl_editsourceform'].handler.getForm()
		...
		setTimeout(()=>{
			var checkcolumns = document.querySelectorAll('#pnl_editsourcegrid-tbl_list .rowcheck');
			...
			var selectbutton = document.getElementById('pnl_editsourcegrid-tbl_list-selectall-button');
			if (detilform.AllowRemoveRecord) {
...
function grd_list_rowclick(tr, ev) {
	...
	$ui.getPages().show('pnl_editsourceform', () => {
		$ui.getPages().ITEMS['pnl_editsourceform'].handler.open(record, trid, header_data)
	})	
}
		...
		yes: () => {
			grd_list.removechecked({
				OnRemoving : async (options) => {
					var apiurl = `${global.modulefullname}/source-delete`
...
function btn_addrow_click() {
	$ui.getPages().show('pnl_editsourceform', ()=>{
		$ui.getPages().ITEMS['pnl_editsourceform'].handler.createnew(header_data)
	})	
}
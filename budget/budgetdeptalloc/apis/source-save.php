...
if (is_file(__DIR__ .'/data-source-handler.php')) {
	require_once __DIR__ .'/data-source-handler.php';
}
...
/**
 * finact/budget/budgetdeptalloc/apis/source-save.php
 *
 * ==========
 * Detil-Save
 * ==========
 * Menampilkan satu baris data/record sesuai PrimaryKey,
 * dari tabel source budgetdeptalloc (mst_budgetdeptallocsource)
 *
...
$API = new class extends budgetdeptallocBase {
	...
	public function execute($data, $options) {
		$event = 'on-save';
		$tablename = 'mst_budgetdeptallocsource';
		$primarykey = 'budgetdeptallocsource_id';
		$autoid = $options->autoid;
		$datastate = $data->_state;

		$userdata = $this->auth->session_get_user();

		$handlerclassname = "\\FGTA4\\apis\\budgetdeptalloc_sourceHandler";
		if (class_exists($handlerclassname)) {
			$hnd = new budgetdeptalloc_sourceHandler($data, $options);
			$hnd->caller = &$this;
			$hnd->db = &$this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			$sqlUpdateField  = [
					'budgetdeptallocsource_id', 'dept_id', 'coabudget_id', 'budgetdeptmonth_year',
					'budgetdeptmonth_month', 'budgetdeptallocsource_value', 'budgetdeptallocsource_apllied', 'owner_dept_id',
					'budgetdeptalloc_id'
			];
			...
			try {
				...
				$header_table = 'mst_budgetdeptalloc';
				$header_primarykey = 'budgetdeptalloc_id';
				$detil_primarykey = 'budgetdeptalloc_id';
				...
				// result
				$options->criteria = [
					"budgetdeptallocsource_id" => $obj->budgetdeptallocsource_id
				];

				$criteriaValues = [
					"budgetdeptallocsource_id" => " budgetdeptallocsource_id = :budgetdeptallocsource_id "
				];
				...
				$sqlFieldList = [
					'budgetdeptallocsource_id' => 'A.`budgetdeptallocsource_id`', 'dept_id' => 'A.`dept_id`', 'coabudget_id' => 'A.`coabudget_id`', 'budgetdeptmonth_year' => 'A.`budgetdeptmonth_year`',
					'budgetdeptmonth_month' => 'A.`budgetdeptmonth_month`', 'budgetdeptallocsource_value' => 'A.`budgetdeptallocsource_value`', 'budgetdeptallocsource_apllied' => 'A.`budgetdeptallocsource_apllied`', 'owner_dept_id' => 'A.`owner_dept_id`',
					'budgetdeptalloc_id' => 'A.`budgetdeptalloc_id`', '_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`', '_modifyby' => 'A.`_modifyby`',
					'_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`', '_modifyby' => 'A.`_modifyby`', '_modifydate' => 'A.`_modifydate`'
				];
				$sqlFromTable = "mst_budgetdeptallocsource A";
				$sqlWhere = $where->sql;

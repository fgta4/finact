...
/**
 * finact/budget/budgetdeptrev/apis/items-save.php
 *
 * ==========
 * Detil-Save
 * ==========
...
$API = new class extends budgetdeptrevBase {
	...
	public function execute($data, $options) {
		...
		if (class_exists($handlerclassname)) {
			$hnd = new budgetdeptrev_itemsHandler($data, $options);
			$hnd->caller = &$this;
			$hnd->db = &$this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			$sqlUpdateField  = [
					'budgetdeptrevitem_id', 'dept_id', 'coabudget_id', 'budgetdeptmonth_year',
					'budgetdeptmonth_month', 'budgetdeptmonth_value', 'owner_dept_id', 'budgetdeptrev_id'
			];
			...
			try {
				...
				$header_table = 'mst_budgetdeptrev';
				$header_primarykey = 'budgetdeptrev_id';
				$detil_primarykey = 'budgetdeptrev_id';
				...
				// result
				$options->criteria = [
					"budgetdeptrevitem_id" => $obj->budgetdeptrevitem_id
				];

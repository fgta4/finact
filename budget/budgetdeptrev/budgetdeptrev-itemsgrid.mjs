...
import * as hnd from  './budgetdeptrev-itemsgrid-hnd.mjs'
...
export function createnew(data, options) {
	...
	txt_title.html(data.budgetdeptrev_descr)
...
export function OpenDetil(data) {
	...
	var fn_listloading = async (options) => {
		...
		options.criteria['id'] = data.budgetdeptrev_id
	...
	var fn_listloaded = async (result, options) => {
		...
		setTimeout(()=>{
			var checkcolumns = document.querySelectorAll('#pnl_edititemsgrid-tbl_list .rowcheck');
			...
			var selectbutton = document.getElementById('pnl_edititemsgrid-tbl_list-selectall-button');
			if (detilform.AllowRemoveRecord) {

...
import * as hnd from  './coabudget-deptgrid-hnd.mjs'
...
export function createnew(data, options) {
	...
	txt_title.html(data.coabudget_name)
...
export function OpenDetil(data) {
	...
	var fn_listloading = async (options) => {
		...
		options.criteria['id'] = data.coabudget_id
	...
	var fn_listloaded = async (result, options) => {
		...
		setTimeout(()=>{
			var checkcolumns = document.querySelectorAll('#pnl_editdeptgrid-tbl_list .rowcheck');
			...
			var selectbutton = document.getElementById('pnl_editdeptgrid-tbl_list-selectall-button');
			if (detilform.AllowRemoveRecord) {

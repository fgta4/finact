...
/**
 * finact/budget/coabudget/apis/delete.php
 *
 * ======
 * Delete
 * ======
 * Menghapus satu baris data/record berdasarkan PrimaryKey
 * pada tabel header coabudget (mst_coabudget)
 *
...
$API = new class extends coabudgetBase {
	...
	public function execute($data, $options) {
		$tablename = 'mst_coabudget';
		$primarykey = 'coabudget_id';

		$userdata = $this->auth->session_get_user();
		$handlerclassname = "\\FGTA4\\apis\\coabudget_headerHandler";
		$hnd = null;
		if (class_exists($handlerclassname)) {
			$hnd = new coabudget_headerHandler($options);
			$hnd->caller = &$this;
			$hnd->db = &$this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			try {
				...
				$tabletodelete = ['mst_coabudgetdept'];

...
import * as hnd from  './budgetdeptprop-itemgrid-hnd.mjs'
...
export function createnew(data, options) {
	...
	txt_title.html(data.budgetdeptprop_descr)
...
export function OpenDetil(data) {
	...
	var fn_listloading = async (options) => {
		...
		options.criteria['id'] = data.budgetdeptprop_id
	...
	var fn_listloaded = async (result, options) => {
		...
		setTimeout(()=>{
			var checkcolumns = document.querySelectorAll('#pnl_edititemgrid-tbl_list .rowcheck');
			...
			var selectbutton = document.getElementById('pnl_edititemgrid-tbl_list-selectall-button');
			if (detilform.AllowRemoveRecord) {

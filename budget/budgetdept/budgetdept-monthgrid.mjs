...
import * as hnd from  './budgetdept-monthgrid-hnd.mjs'

const tbl_list = $('#pnl_editmonthgrid-tbl_list');
const txt_title = $('#pnl_editmonthgrid-title');
const pnl_control = $('.pnl_editmonthgrid-control');
const btn_removechecked  = $('#pnl_editmonthgrid-removechecked');
const btn_addrow = $('#pnl_editmonthgrid-addrow');
...
export function OpenDetil(data) {
	...
	var fn_listloading = async (options) => {
		options.api = `${global.modulefullname}/month-list`
		options.criteria['id'] = data.budgetdeptyear_id
	...
	var fn_listloaded = async (result, options) => {
		...
		var detilform = $ui.getPages().ITEMS['pnl_editmonthform'].handler.getForm()
		...
		setTimeout(()=>{
			var checkcolumns = document.querySelectorAll('#pnl_editmonthgrid-tbl_list .rowcheck');
			...
			var selectbutton = document.getElementById('pnl_editmonthgrid-tbl_list-selectall-button');
			if (detilform.AllowRemoveRecord) {
...
function grd_list_rowclick(tr, ev) {
	...
	$ui.getPages().show('pnl_editmonthform', () => {
		$ui.getPages().ITEMS['pnl_editmonthform'].handler.open(record, trid, header_data)
	})	
}
		...
		yes: () => {
			grd_list.removechecked({
				OnRemoving : async (options) => {
					var apiurl = `${global.modulefullname}/month-delete`
...
function btn_addrow_click() {
	$ui.getPages().show('pnl_editmonthform', ()=>{
		$ui.getPages().ITEMS['pnl_editmonthform'].handler.createnew(header_data)
	})	
}
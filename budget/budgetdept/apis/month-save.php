...
if (is_file(__DIR__ .'/data-month-handler.php')) {
	require_once __DIR__ .'/data-month-handler.php';
}
...
/**
 * finact/budget/budgetdept/apis/month-save.php
 *
 * ==========
 * Detil-Save
 * ==========
 * Menampilkan satu baris data/record sesuai PrimaryKey,
 * dari tabel month budgetdept (mst_budgetdeptmonth)
 *
 ...
 * tanggal 06/02/2024
 */
$API = new class extends budgetdeptBase {
	
	public function execute($data, $options) {
		$event = 'on-save';
		$tablename = 'mst_budgetdeptmonth';
		$primarykey = 'budgetdeptmonth_id';
		$autoid = $options->autoid;
		$datastate = $data->_state;

		$userdata = $this->auth->session_get_user();

		$handlerclassname = "\\FGTA4\\apis\\budgetdept_monthHandler";
		if (class_exists($handlerclassname)) {
			$hnd = new budgetdept_monthHandler($data, $options);
			$hnd->caller = &$this;
			$hnd->db = &$this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			$sqlUpdateField  = [
					'budgetdeptmonth_id', 'dept_id', 'coabudget_id', 'budgetdeptmonth_year',
					'budgetdeptmonth_month', 'budgetdeptmonth_value', 'owner_dept_id', 'budgetdeptprop_id',
					'budgetdeptpropitem_id', 'budgetdeptyear_id'
			];
			...
			if ($obj->budgetdeptprop_id=='') { $obj->budgetdeptprop_id = '--NULL--'; }
			if ($obj->budgetdeptpropitem_id=='') { $obj->budgetdeptpropitem_id = '--NULL--'; }



			unset($obj->budgetdeptprop_id);
			unset($obj->budgetdeptpropitem_id);
			...
			try {
				...
				$header_table = 'mst_budgetdeptyear';
				$header_primarykey = 'budgetdeptyear_id';
				$detil_primarykey = 'budgetdeptyear_id';
				...
				// result
				$options->criteria = [
					"budgetdeptmonth_id" => $obj->budgetdeptmonth_id
				];

				$criteriaValues = [
					"budgetdeptmonth_id" => " budgetdeptmonth_id = :budgetdeptmonth_id "
				];
				...
				$sqlFieldList = [
					'budgetdeptmonth_id' => 'A.`budgetdeptmonth_id`', 'dept_id' => 'A.`dept_id`', 'coabudget_id' => 'A.`coabudget_id`', 'budgetdeptmonth_year' => 'A.`budgetdeptmonth_year`',
					'budgetdeptmonth_month' => 'A.`budgetdeptmonth_month`', 'budgetdeptmonth_value' => 'A.`budgetdeptmonth_value`', 'owner_dept_id' => 'A.`owner_dept_id`', 'budgetdeptprop_id' => 'A.`budgetdeptprop_id`',
					'budgetdeptpropitem_id' => 'A.`budgetdeptpropitem_id`', 'budgetdeptyear_id' => 'A.`budgetdeptyear_id`', '_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`',
					'_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`', '_modifyby' => 'A.`_modifyby`', '_modifydate' => 'A.`_modifydate`'
				];
				$sqlFromTable = "mst_budgetdeptmonth A";
				$sqlWhere = $where->sql;

...
class PrintForm extends WebModule {
	function __construct() {
		// $logfilepath = __LOCALDB_DIR . "/output/logfile-itemassetmove.txt";
	...
	public function LoadPage() {
		...
		try {
			...
			$stmt->execute([':itemassetmove_id' => $id]);
			$rows  = $stmt->fetchall(\PDO::FETCH_ASSOC);
			$row = $rows[0];

			$this->itemassetmove_id = $row['itemassetmove_id'];
	...
	function getSqlHeader() {
		return "
			...
			from trn_itemassetmove A where itemassetmove_id = :itemassetmove_id
		";
	}	

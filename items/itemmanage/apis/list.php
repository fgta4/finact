...
/**
 * finact/items/itemmanage/apis/list.php
 *
 * ========
 * DataList
 * ========
 * Menampilkan data-data pada tabel header itemmanage (mst_itemmanage)
 ...
 * tanggal 09/02/2023
 */
$API = new class extends itemmanageBase {

	public function execute($options) {

		$userdata = $this->auth->session_get_user();

		$handlerclassname = "\\FGTA4\\apis\\itemmanage_headerHandler";
		if (class_exists($handlerclassname)) {
			$hnd = new itemmanage_headerHandler($options);
			$hnd->caller = &$this;
			$hnd->db = $this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			$criteriaValues = [
				"search" => " A.itemmanage_id LIKE CONCAT('%', :search, '%') OR A.itemmanage_name LIKE CONCAT('%', :search, '%') "
			];
			...
			$sqlFieldList = [
				'itemmanage_id' => 'A.`itemmanage_id`', 'itemmanage_name' => 'A.`itemmanage_name`', 'itemmanage_isasset' => 'A.`itemmanage_isasset`', 'itemmanage_isbyassetowner' => 'A.`itemmanage_isbyassetowner`',

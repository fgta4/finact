...
/**
 * finact/items/itemclass/apis/files-save.php
 *
 * ==========
 * Detil-Save
 * ==========
 * Menampilkan satu baris data/record sesuai PrimaryKey,
 * dari tabel files itemclass (mst_itemclassfiles)
 *
...
$API = new class extends itemclassBase {
	...
	public function execute($data, $options, $files) {
		$event = 'on-save';
		$tablename = 'mst_itemclassfiles';
		$primarykey = 'itemclassfiles_id';
		$autoid = $options->autoid;
		$datastate = $data->_state;

		$userdata = $this->auth->session_get_user();

		$handlerclassname = "\\FGTA4\\apis\\itemclass_filesHandler";
		if (class_exists($handlerclassname)) {
			$hnd = new itemclass_filesHandler($data, $options);
			$hnd->caller = &$this;
			$hnd->db = &$this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			$sqlUpdateField  = [
					'itemclassfiles_id', 'doctype_id', 'itemclassfiles_descr', 'itemclassfiles_order',
					'itemclassfiles_file', 'itemclass_id'
			];
			...
			try {
				...
				if (property_exists($files, $fieldname)) {
					...
					$objfile->itemclassfiles_file = $rev;
				...
				// result
				$options->criteria = [
					"itemclassfiles_id" => $obj->itemclassfiles_id
				];

				$criteriaValues = [
					"itemclassfiles_id" => " itemclassfiles_id = :itemclassfiles_id "
				];
				...
				$sqlFieldList = [
					'itemclassfiles_id' => 'A.`itemclassfiles_id`', 'doctype_id' => 'A.`doctype_id`', 'itemclassfiles_descr' => 'A.`itemclassfiles_descr`', 'itemclassfiles_order' => 'A.`itemclassfiles_order`',
					'itemclassfiles_file' => 'A.`itemclassfiles_file`', 'itemclass_id' => 'A.`itemclass_id`', '_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`',
					'_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`', '_modifyby' => 'A.`_modifyby`', '_modifydate' => 'A.`_modifydate`'
				];
				$sqlFromTable = "mst_itemclassfiles A";
				$sqlWhere = $where->sql;
				...
				$dataresponse = array_merge($record, [
					...
					'doctype_name' => \FGTA4\utils\SqlUtility::Lookup($record['doctype_id'], $this->db, 'mst_doctype', 'doctype_id', 'doctype_name'),

...
import * as hnd from  './itemclass-accountgrid-hnd.mjs'

const tbl_list = $('#pnl_editaccountgrid-tbl_list');
const txt_title = $('#pnl_editaccountgrid-title');
const pnl_control = $('.pnl_editaccountgrid-control');
const btn_removechecked  = $('#pnl_editaccountgrid-removechecked');
const btn_addrow = $('#pnl_editaccountgrid-addrow');
...
function grd_list_rowclick(tr, ev) {
	...
	$ui.getPages().show('pnl_editaccountform', () => {
		$ui.getPages().ITEMS['pnl_editaccountform'].handler.open(record, trid, header_data)
	})	
}
		...
		yes: () => {
			grd_list.removechecked({
				OnRemoving : async (options) => {
					var apiurl = `${global.modulefullname}/account-delete`
...
function btn_addrow_click() {
	$ui.getPages().show('pnl_editaccountform', ()=>{
		$ui.getPages().ITEMS['pnl_editaccountform'].handler.createnew(header_data)
	})	
}
...
const tbl_list = $('#pnl_editspecgrid-tbl_list');
const txt_title = $('#pnl_editspecgrid-title');
const pnl_control = $('#pnl_editspecgrid-control');
const btn_removechecked  = $('#pnl_editspecgrid-removechecked');
const btn_addrow = $('#pnl_editspecgrid-addrow');
...
export function OpenDetil(data) {
	...
	var fn_listloading = async (options) => {
		options.api = `${global.modulefullname}/spec-list`
	...
	var fn_listloaded = async (result, options) => {
		...
		var detilform = $ui.getPages().ITEMS['pnl_editspecform'].handler.getForm()
...
function grd_list_rowclick(tr, ev) {
	...
	$ui.getPages().show('pnl_editspecform', () => {
		$ui.getPages().ITEMS['pnl_editspecform'].handler.open(record, trid, header_data)
	})	
}
		...
		yes: () => {
			grd_list.removechecked({
				OnRemoving : async (options) => {
					var apiurl = `${global.modulefullname}/spec-delete`
...
function btn_addrow_click() {
	$ui.getPages().show('pnl_editspecform', ()=>{
		$ui.getPages().ITEMS['pnl_editspecform'].handler.createnew(header_data)
	})	
}
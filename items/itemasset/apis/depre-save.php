...
/**
 * finact/items/itemasset/apis/depre-save.php
 *
 * ==========
 * Detil-Save
 * ==========
 * Menampilkan satu baris data/record sesuai PrimaryKey,
 * dari tabel depre itemasset (mst_itemasset)
 *
...
$API = new class extends itemassetBase {
	...
	public function execute($data, $options, $files) {
		...
		try {
			...
			$obj->itemassetdepre_date = (\DateTime::createFromFormat('d/m/Y',$obj->itemassetdepre_date))->format('Y-m-d');

			$obj->itemassetdepre_ref = strtoupper($obj->itemassetdepre_ref);
			$obj->itemasset_id = strtoupper($obj->itemasset_id);


			if ($obj->itemassetdepre_ref=='') { $obj->itemassetdepre_ref = '--NULL--'; }
			...
			try {
				...
				$sql = \FGTA4\utils\SqlUtility::Select($tablename , [
					$primarykey
					, 'itemassetdepre_id', 'itemassetdepre_date', 'itemassetdepre_value', 'itemassetdepre_ref', 'itemasset_id', '_createby', '_createdate', '_modifyby', '_modifydate', '_createby', '_createdate', '_modifyby', '_modifydate'
				], $where->sql);
				...
				'itemassetdepre_date' => date("d/m/Y", strtotime($row['itemassetdepre_date'])),

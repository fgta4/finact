...
const pnl_control = $('#pnl_editdepregrid-control');
const btn_removechecked  = $('#pnl_editdepregrid-removechecked');
const btn_addrow = $('#pnl_editdepregrid-addrow');
...
export function OpenDetil(data) {
	...
	var fn_listloading = async (options) => {
		options.api = `${global.modulefullname}/depre-list`
	...
	var fn_listloaded = async (result, options) => {
		...
		var detilform = $ui.getPages().ITEMS['pnl_editdepreform'].handler.getForm()
...
function grd_list_rowclick(tr, ev) {
	...
	$ui.getPages().show('pnl_editdepreform', () => {
		$ui.getPages().ITEMS['pnl_editdepreform'].handler.open(record, trid, header_data)
	})	
}
		...
		yes: () => {
			grd_list.removechecked({
				OnRemoving : async (options) => {
					var apiurl = `${global.modulefullname}/depre-delete`
...
function btn_addrow_click() {
	$ui.getPages().show('pnl_editdepreform', ()=>{
		$ui.getPages().ITEMS['pnl_editdepreform'].handler.createnew(header_data)
	})	
}
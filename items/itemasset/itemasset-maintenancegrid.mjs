...
const tbl_list = $('#pnl_editmaintenancegrid-tbl_list');
const txt_title = $('#pnl_editmaintenancegrid-title');
const pnl_control = $('#pnl_editmaintenancegrid-control');
const btn_removechecked  = $('#pnl_editmaintenancegrid-removechecked');
const btn_addrow = $('#pnl_editmaintenancegrid-addrow');
...
export function createnew(data, options) {
	...
	grd_list.clear();
	txt_title.html(data.itemasset_name)
	header_data = data;
}
...
export function OpenDetil(data) {
	...
	var fn_listloading = async (options) => {
		options.api = `${global.modulefullname}/maintenance-list`
		options.criteria['id'] = data.itemasset_id
	}
	var fn_listloaded = async (result, options) => {
		...
		var detilform = $ui.getPages().ITEMS['pnl_editmaintenanceform'].handler.getForm()
...
function grd_list_rowclick(tr, ev) {
	...
	$ui.getPages().show('pnl_editmaintenanceform', () => {
		$ui.getPages().ITEMS['pnl_editmaintenanceform'].handler.open(record, trid, header_data)
	})	
}
		...
		yes: () => {
			grd_list.removechecked({
				OnRemoving : async (options) => {
					var apiurl = `${global.modulefullname}/maintenance-delete`
...
function btn_addrow_click() {
	$ui.getPages().show('pnl_editmaintenanceform', ()=>{
		$ui.getPages().ITEMS['pnl_editmaintenanceform'].handler.createnew(header_data)
	})	
}
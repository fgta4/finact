...
const tbl_list = $('#pnl_editbookinggrid-tbl_list');
const txt_title = $('#pnl_editbookinggrid-title');
const pnl_control = $('#pnl_editbookinggrid-control');
const btn_removechecked  = $('#pnl_editbookinggrid-removechecked');
const btn_addrow = $('#pnl_editbookinggrid-addrow');
...
export function OpenDetil(data) {
	...
	var fn_listloading = async (options) => {
		options.api = `${global.modulefullname}/booking-list`
	...
	var fn_listloaded = async (result, options) => {
		...
		var detilform = $ui.getPages().ITEMS['pnl_editbookingform'].handler.getForm()
...
function grd_list_rowclick(tr, ev) {
	...
	$ui.getPages().show('pnl_editbookingform', () => {
		$ui.getPages().ITEMS['pnl_editbookingform'].handler.open(record, trid, header_data)
	})	
}
		...
		yes: () => {
			grd_list.removechecked({
				OnRemoving : async (options) => {
					var apiurl = `${global.modulefullname}/booking-delete`
...
function btn_addrow_click() {
	$ui.getPages().show('pnl_editbookingform', ()=>{
		$ui.getPages().ITEMS['pnl_editbookingform'].handler.createnew(header_data)
	})	
}
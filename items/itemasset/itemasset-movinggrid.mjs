...
const tbl_list = $('#pnl_editmovinggrid-tbl_list');
const txt_title = $('#pnl_editmovinggrid-title');
const pnl_control = $('#pnl_editmovinggrid-control');
const btn_removechecked  = $('#pnl_editmovinggrid-removechecked');
const btn_addrow = $('#pnl_editmovinggrid-addrow');
...
export function OpenDetil(data) {
	...
	var fn_listloading = async (options) => {
		options.api = `${global.modulefullname}/moving-list`
	...
	var fn_listloaded = async (result, options) => {
		...
		var detilform = $ui.getPages().ITEMS['pnl_editmovingform'].handler.getForm()
...
function grd_list_rowclick(tr, ev) {
	...
	$ui.getPages().show('pnl_editmovingform', () => {
		$ui.getPages().ITEMS['pnl_editmovingform'].handler.open(record, trid, header_data)
	})	
}
		...
		yes: () => {
			grd_list.removechecked({
				OnRemoving : async (options) => {
					var apiurl = `${global.modulefullname}/moving-delete`
...
function btn_addrow_click() {
	$ui.getPages().show('pnl_editmovingform', ()=>{
		$ui.getPages().ITEMS['pnl_editmovingform'].handler.createnew(header_data)
	})	
}
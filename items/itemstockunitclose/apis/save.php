...
/**
 * finact/items/itemstockunitclose/apis/save.php
 *
 * ====
 * Save
 * ====
...
$API = new class extends itemstockunitcloseBase {
	...
	public function execute($data, $options) {
		...
		try {
			...
			$obj->itemstockunit_closedate = (\DateTime::createFromFormat('d/m/Y',$obj->itemstockunit_closedate))->format('Y-m-d');



			if ($obj->itemstockunit_closeby=='') { $obj->itemstockunit_closeby = '--NULL--'; }
			if ($obj->itemstockunit_closedate=='') { $obj->itemstockunit_closedate = '--NULL--'; }


			unset($obj->itemstockunit_isclosed);
			unset($obj->itemstockunit_closeby);
			unset($obj->itemstockunit_closedate);
			...
			try {
				...
				$result->dataresponse = (object) array_merge($record, [
					...
					'itemstockunit_closedate' => date("d/m/Y", strtotime($row['itemstockunit_closedate'])),

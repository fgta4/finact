...
import * as hnd from  './itemstock-propgrid-hnd.mjs'

const tbl_list = $('#pnl_editpropgrid-tbl_list');
const txt_title = $('#pnl_editpropgrid-title');
const pnl_control = $('.pnl_editpropgrid-control');
const btn_removechecked  = $('#pnl_editpropgrid-removechecked');
const btn_addrow = $('#pnl_editpropgrid-addrow');
...
export function OpenDetil(data) {
	...
	var fn_listloading = async (options) => {
		options.api = `${global.modulefullname}/prop-list`
	...
	var fn_listloaded = async (result, options) => {
		...
		var detilform = $ui.getPages().ITEMS['pnl_editpropform'].handler.getForm()
		...
		setTimeout(()=>{
			var checkcolumns = document.querySelectorAll('#pnl_editpropgrid-tbl_list .rowcheck');
			...
			var selectbutton = document.getElementById('pnl_editpropgrid-tbl_list-selectall-button');
			if (detilform.AllowRemoveRecord) {
...
function grd_list_rowclick(tr, ev) {
	...
	$ui.getPages().show('pnl_editpropform', () => {
		$ui.getPages().ITEMS['pnl_editpropform'].handler.open(record, trid, header_data)
	})	
}
		...
		yes: () => {
			grd_list.removechecked({
				OnRemoving : async (options) => {
					var apiurl = `${global.modulefullname}/prop-delete`
...
function btn_addrow_click() {
	$ui.getPages().show('pnl_editpropform', ()=>{
		$ui.getPages().ITEMS['pnl_editpropform'].handler.createnew(header_data)
	})	
}
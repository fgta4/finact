...
const tbl_list = $('#pnl_editsettinggrid-tbl_list');
const txt_title = $('#pnl_editsettinggrid-title');
const pnl_control = $('.pnl_editsettinggrid-control');
const btn_removechecked  = $('#pnl_editsettinggrid-removechecked');
const btn_addrow = $('#pnl_editsettinggrid-addrow');
...
export function OpenDetil(data) {
	...
	var fn_listloading = async (options) => {
		options.api = `${global.modulefullname}/setting-list`
	...
	var fn_listloaded = async (result, options) => {
		...
		var detilform = $ui.getPages().ITEMS['pnl_editsettingform'].handler.getForm()
...
function grd_list_rowclick(tr, ev) {
	...
	$ui.getPages().show('pnl_editsettingform', () => {
		$ui.getPages().ITEMS['pnl_editsettingform'].handler.open(record, trid, header_data)
	})	
}
		...
		yes: () => {
			grd_list.removechecked({
				OnRemoving : async (options) => {
					var apiurl = `${global.modulefullname}/setting-delete`
...
function btn_addrow_click() {
	$ui.getPages().show('pnl_editsettingform', ()=>{
		$ui.getPages().ITEMS['pnl_editsettingform'].handler.createnew(header_data)
	})	
}
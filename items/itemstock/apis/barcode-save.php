...
if (is_file(__DIR__ .'/data-barcode-handler.php')) {
	require_once __DIR__ .'/data-barcode-handler.php';
}
...
/**
 * finact/items/itemstock/apis/barcode-save.php
 *
 * ==========
 * Detil-Save
 * ==========
 * Menampilkan satu baris data/record sesuai PrimaryKey,
 * dari tabel barcode itemstock (mst_itemstockbarcode)
 *
...
$API = new class extends itemstockBase {
	...
	public function execute($data, $options, $files) {
		...
		$handlerclassname = "\\FGTA4\\apis\\itemstock_barcodeHandler";
		if (class_exists($handlerclassname)) {
			$hnd = new itemstock_barcodeHandler($data, $options);
			$hnd->caller = &$this;
			$hnd->db = &$this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			$sqlUpdateField  = [
					'itemstockbarcode_id', 'itemstockbarcode_text', 'brand_id', 'itemstock_id'
			];
			...
			if ($obj->itemstockbarcode_text=='') { $obj->itemstockbarcode_text = '--NULL--'; }
			...
			try {
				...
				// result
				$options->criteria = [
					"itemstockbarcode_id" => $obj->itemstockbarcode_id
				];

				$criteriaValues = [
					"itemstockbarcode_id" => " itemstockbarcode_id = :itemstockbarcode_id "
				];
				...
				$sqlFieldList = [
					'itemstockbarcode_id' => 'A.`itemstockbarcode_id`', 'itemstockbarcode_text' => 'A.`itemstockbarcode_text`', 'brand_id' => 'A.`brand_id`', 'itemstock_id' => 'A.`itemstock_id`',
					'_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`', '_modifyby' => 'A.`_modifyby`', '_modifydate' => 'A.`_modifydate`'
				];
				$sqlFromTable = "mst_itemstockbarcode A";
				$sqlWhere = $where->sql;

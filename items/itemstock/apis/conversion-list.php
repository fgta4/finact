...
/**
 * finact/items/itemstock/apis/conversion-list.php
 *
 * ==============
 * Detil-DataList
 * ==============
 * Menampilkan data-data pada tabel conversion itemstock (mst_itemstockconversion)
...
$API = new class extends itemstockBase {
	...
	public function execute($options) {
		...
		if (class_exists($handlerclassname)) {
			$hnd = new itemstock_conversionHandler($options);
			$hnd->caller = $this;
			$hnd->db = $this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			foreach ($rows as $row) {
				...
				*/
				...
				$this->addFields('itemstock_conversion_name', 'itemstock_conversion', $record, 'mst_itemstock', 'itemstock_name', 'itemstock_id');

...
if (is_file(__DIR__ .'/data-position-handler.php')) {
	require_once __DIR__ .'/data-position-handler.php';
}
...
/**
 * finact/items/itemstock/apis/position-save.php
 *
 * ==========
 * Detil-Save
 * ==========
 * Menampilkan satu baris data/record sesuai PrimaryKey,
 * dari tabel position itemstock (mst_itemstockposition)
 *
...
$API = new class extends itemstockBase {
	...
	public function execute($data, $options, $files) {
		$event = 'on-save';
		$tablename = 'mst_itemstockposition';
		$primarykey = 'itemstockposition_id';
		$autoid = $options->autoid;
		$datastate = $data->_state;

		$userdata = $this->auth->session_get_user();

		$handlerclassname = "\\FGTA4\\apis\\itemstock_positionHandler";
		if (class_exists($handlerclassname)) {
			$hnd = new itemstock_positionHandler($data, $options);
			$hnd->caller = &$this;
			$hnd->db = &$this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			$sqlUpdateField  = [
					'itemstockposition_id', 'itemstockposition_date', 'unit_id', 'site_id',
					'dept_id', 'brand_id', 'unitmeasurement_id', 'itemstocksaldo_qty',
					'itemstocksaldo_value', 'itemstockposition_isupdating', 'itemstockposition_updatebatch', 'itemstock_id'
			];
			...
			if ($obj->site_id=='') { $obj->site_id = '--NULL--'; }
			if ($obj->dept_id=='') { $obj->dept_id = '--NULL--'; }
			if ($obj->brand_id=='') { $obj->brand_id = '--NULL--'; }
			if ($obj->unitmeasurement_id=='') { $obj->unitmeasurement_id = '--NULL--'; }
			if ($obj->itemstockposition_updatebatch=='') { $obj->itemstockposition_updatebatch = '--NULL--'; }



			unset($obj->itemstockposition_date);
			unset($obj->unit_id);
			unset($obj->site_id);
			unset($obj->dept_id);
			unset($obj->brand_id);
			unset($obj->unitmeasurement_id);
			...
			try {
				...
				// result
				$options->criteria = [
					"itemstockposition_id" => $obj->itemstockposition_id
				];

				$criteriaValues = [
					"itemstockposition_id" => " itemstockposition_id = :itemstockposition_id "
				];
				...
				$sqlFieldList = [
					'itemstockposition_id' => 'A.`itemstockposition_id`', 'itemstockposition_date' => 'A.`itemstockposition_date`', 'unit_id' => 'A.`unit_id`', 'site_id' => 'A.`site_id`',
					'dept_id' => 'A.`dept_id`', 'brand_id' => 'A.`brand_id`', 'unitmeasurement_id' => 'A.`unitmeasurement_id`', 'itemstocksaldo_qty' => 'A.`itemstocksaldo_qty`',
					'itemstocksaldo_value' => 'A.`itemstocksaldo_value`', 'itemstockposition_isupdating' => 'A.`itemstockposition_isupdating`', 'itemstockposition_updatebatch' => 'A.`itemstockposition_updatebatch`', 'itemstock_id' => 'A.`itemstock_id`',
					'_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`', '_modifyby' => 'A.`_modifyby`', '_modifydate' => 'A.`_modifydate`'
				];
				$sqlFromTable = "mst_itemstockposition A";
				$sqlWhere = $where->sql;

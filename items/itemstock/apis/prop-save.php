...
/**
 * finact/items/itemstock/apis/prop-save.php
 *
 * ==========
 * Detil-Save
 * ==========
 * Menampilkan satu baris data/record sesuai PrimaryKey,
 * dari tabel prop itemstock (mst_itemstockprop)
 *
...
$API = new class extends itemstockBase {
	...
	public function execute($data, $options, $files) {
		$event = 'on-save';
		$tablename = 'mst_itemstockprop';
		$primarykey = 'itemstockprop_id';
		$autoid = $options->autoid;
		$datastate = $data->_state;

		$userdata = $this->auth->session_get_user();

		$handlerclassname = "\\FGTA4\\apis\\itemstock_propHandler";
		if (class_exists($handlerclassname)) {
			$hnd = new itemstock_propHandler($data, $options);
			$hnd->caller = &$this;
			$hnd->db = &$this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			$sqlUpdateField  = [
					'itemstockprop_id', 'itemproptype_id', 'itemstockprop_keys', 'itemstockprop_value',
					'itemstock_id'
			];
			...
			try {
				...
				// result
				$options->criteria = [
					"itemstockprop_id" => $obj->itemstockprop_id
				];

				$criteriaValues = [
					"itemstockprop_id" => " itemstockprop_id = :itemstockprop_id "
				];
				...
				$sqlFieldList = [
					'itemstockprop_id' => 'A.`itemstockprop_id`', 'itemproptype_id' => 'A.`itemproptype_id`', 'itemstockprop_keys' => 'A.`itemstockprop_keys`', 'itemstockprop_value' => 'A.`itemstockprop_value`',
					'itemstock_id' => 'A.`itemstock_id`', '_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`', '_modifyby' => 'A.`_modifyby`',
					'_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`', '_modifyby' => 'A.`_modifyby`', '_modifydate' => 'A.`_modifydate`'
				];
				$sqlFromTable = "mst_itemstockprop A";
				$sqlWhere = $where->sql;
				...
				$dataresponse = array_merge($record, [
					...
					'itemproptype_name' => \FGTA4\utils\SqlUtility::Lookup($record['itemproptype_id'], $this->db, 'mst_itemproptype', 'itemproptype_id', 'itemproptype_name'),

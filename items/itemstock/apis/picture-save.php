...
/**
 * finact/items/itemstock/apis/picture-save.php
 *
 * ==========
 * Detil-Save
 * ==========
...
$API = new class extends itemstockBase {
	...
	public function execute($data, $options, $files) {
		...
		if (class_exists($handlerclassname)) {
			$hnd = new itemstock_pictureHandler($data, $options);
			$hnd->caller = &$this;
			$hnd->db = &$this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			$sqlUpdateField  = [
					'itemstockpic_id', 'itemstockpic_couchdbid', 'itemstockpic_name', 'itemstockpic_descr',
					'itemstockpic_order', 'itemstockpic_file', 'itemstock_id'
			];
			...
			if ($obj->itemstockpic_couchdbid=='') { $obj->itemstockpic_couchdbid = '--NULL--'; }
			...
			try {
				...
				// result
				$options->criteria = [
					"itemstockpic_id" => $obj->itemstockpic_id
				];

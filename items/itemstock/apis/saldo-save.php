...
/**
 * finact/items/itemstock/apis/saldo-save.php
 *
 * ==========
 * Detil-Save
 * ==========
...
$API = new class extends itemstockBase {
	...
	public function execute($data, $options) {
		...
		try {
			...
			if ($obj->periodemo_id=='') { $obj->periodemo_id = '--NULL--'; }
			if ($obj->unit_id=='') { $obj->unit_id = '--NULL--'; }
			if ($obj->itemstockunitclose_id=='') { $obj->itemstockunitclose_id = '--NULL--'; }
			...
			unset($obj->unit_id);
			unset($obj->itemstockunitclose_id);
			unset($obj->itemstocksaldo_date);
			unset($obj->itemstocksaldo_dategen);
			unset($obj->site_id);
			unset($obj->dept_id);
			unset($obj->brand_id);
			...
			try {
				...
				$sql = \FGTA4\utils\SqlUtility::Select($tablename , [
					$primarykey
					, 'itemstocksaldo_id', 'periodemo_id', 'unit_id', 'itemstockunitclose_id', 'itemstocksaldo_date', 'itemstocksaldo_dategen', 'site_id', 'dept_id', 'brand_id', 'unitmeasurement_id', 'itemstock_id', 'itemstocksaldo_qty', 'itemstocksaldo_value', '_createby', '_createdate', '_modifyby', '_modifydate', '_createby', '_createdate', '_modifyby', '_modifydate'
				], $where->sql);

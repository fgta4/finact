...
/**
 * finact/items/itemstock/apis/prop-list.php
 *
 * ==============
 * Detil-DataList
 * ==============
 * Menampilkan data-data pada tabel prop itemstock (mst_itemstockprop)
...
$API = new class extends itemstockBase {
	...
	public function execute($options) {
		...
		if (class_exists($handlerclassname)) {
			$hnd = new itemstock_propHandler($options);
			$hnd->caller = $this;
			$hnd->db = $this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			foreach ($rows as $row) {
				...
				*/
				...
				$this->addFields('itemproptype_name', 'itemproptype_id', $record, 'mst_itemproptype', 'itemproptype_name', 'itemproptype_id');

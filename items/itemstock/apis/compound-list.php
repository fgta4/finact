...
/**
 * finact/items/itemstock/apis/compound-list.php
 *
 * ==============
 * Detil-DataList
 * ==============
 * Menampilkan data-data pada tabel compound itemstock (mst_itemstockcompound)
...
$API = new class extends itemstockBase {
	...
	public function execute($options) {
		...
		if (class_exists($handlerclassname)) {
			$hnd = new itemstock_compoundHandler($options);
			$hnd->caller = $this;
			$hnd->db = $this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			foreach ($rows as $row) {
				...
				*/
				...
				$this->addFields('itemstock_compound_name', 'itemstock_compound', $record, 'mst_itemstock', 'itemstock_name', 'itemstock_id');

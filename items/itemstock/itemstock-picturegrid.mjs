...
import * as hnd from  './itemstock-picturegrid-hnd.mjs'

const tbl_list = $('#pnl_editpicturegrid-tbl_list');
const txt_title = $('#pnl_editpicturegrid-title');
const pnl_control = $('.pnl_editpicturegrid-control');
const btn_removechecked  = $('#pnl_editpicturegrid-removechecked');
const btn_addrow = $('#pnl_editpicturegrid-addrow');
...
export function OpenDetil(data) {
	...
	var fn_listloading = async (options) => {
		options.api = `${global.modulefullname}/picture-list`
	...
	var fn_listloaded = async (result, options) => {
		...
		var detilform = $ui.getPages().ITEMS['pnl_editpictureform'].handler.getForm()
		...
		setTimeout(()=>{
			var checkcolumns = document.querySelectorAll('#pnl_editpicturegrid-tbl_list .rowcheck');
			...
			var selectbutton = document.getElementById('pnl_editpicturegrid-tbl_list-selectall-button');
			if (detilform.AllowRemoveRecord) {
...
function grd_list_rowclick(tr, ev) {
	...
	$ui.getPages().show('pnl_editpictureform', () => {
		$ui.getPages().ITEMS['pnl_editpictureform'].handler.open(record, trid, header_data)
	})	
}
		...
		yes: () => {
			grd_list.removechecked({
				OnRemoving : async (options) => {
					var apiurl = `${global.modulefullname}/picture-delete`
...
function btn_addrow_click() {
	$ui.getPages().show('pnl_editpictureform', ()=>{
		$ui.getPages().ITEMS['pnl_editpictureform'].handler.createnew(header_data)
	})	
}
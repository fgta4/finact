...
import * as hnd from  './itemstock-barcodegrid-hnd.mjs'

const tbl_list = $('#pnl_editbarcodegrid-tbl_list');
const txt_title = $('#pnl_editbarcodegrid-title');
const pnl_control = $('.pnl_editbarcodegrid-control');
const btn_removechecked  = $('#pnl_editbarcodegrid-removechecked');
const btn_addrow = $('#pnl_editbarcodegrid-addrow');
...
export function OpenDetil(data) {
	...
	var fn_listloading = async (options) => {
		options.api = `${global.modulefullname}/barcode-list`
	...
	var fn_listloaded = async (result, options) => {
		...
		var detilform = $ui.getPages().ITEMS['pnl_editbarcodeform'].handler.getForm()
		...
		setTimeout(()=>{
			var checkcolumns = document.querySelectorAll('#pnl_editbarcodegrid-tbl_list .rowcheck');
			...
			var selectbutton = document.getElementById('pnl_editbarcodegrid-tbl_list-selectall-button');
			if (detilform.AllowRemoveRecord) {
...
function grd_list_rowclick(tr, ev) {
	...
	$ui.getPages().show('pnl_editbarcodeform', () => {
		$ui.getPages().ITEMS['pnl_editbarcodeform'].handler.open(record, trid, header_data)
	})	
}
		...
		yes: () => {
			grd_list.removechecked({
				OnRemoving : async (options) => {
					var apiurl = `${global.modulefullname}/barcode-delete`
...
function btn_addrow_click() {
	$ui.getPages().show('pnl_editbarcodeform', ()=>{
		$ui.getPages().ITEMS['pnl_editbarcodeform'].handler.createnew(header_data)
	})	
}
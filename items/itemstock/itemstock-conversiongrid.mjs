...
import * as hnd from  './itemstock-conversiongrid-hnd.mjs'

const tbl_list = $('#pnl_editconversiongrid-tbl_list');
const txt_title = $('#pnl_editconversiongrid-title');
const pnl_control = $('.pnl_editconversiongrid-control');
const btn_removechecked  = $('#pnl_editconversiongrid-removechecked');
const btn_addrow = $('#pnl_editconversiongrid-addrow');
...
export function OpenDetil(data) {
	...
	var fn_listloading = async (options) => {
		options.api = `${global.modulefullname}/conversion-list`
	...
	var fn_listloaded = async (result, options) => {
		...
		var detilform = $ui.getPages().ITEMS['pnl_editconversionform'].handler.getForm()
		...
		setTimeout(()=>{
			var checkcolumns = document.querySelectorAll('#pnl_editconversiongrid-tbl_list .rowcheck');
			...
			var selectbutton = document.getElementById('pnl_editconversiongrid-tbl_list-selectall-button');
			if (detilform.AllowRemoveRecord) {
...
function grd_list_rowclick(tr, ev) {
	...
	$ui.getPages().show('pnl_editconversionform', () => {
		$ui.getPages().ITEMS['pnl_editconversionform'].handler.open(record, trid, header_data)
	})	
}
		...
		yes: () => {
			grd_list.removechecked({
				OnRemoving : async (options) => {
					var apiurl = `${global.modulefullname}/conversion-delete`
...
function btn_addrow_click() {
	$ui.getPages().show('pnl_editconversionform', ()=>{
		$ui.getPages().ITEMS['pnl_editconversionform'].handler.createnew(header_data)
	})	
}
...
import * as hnd from  './itemstock-compoundgrid-hnd.mjs'

const tbl_list = $('#pnl_editcompoundgrid-tbl_list');
const txt_title = $('#pnl_editcompoundgrid-title');
const pnl_control = $('.pnl_editcompoundgrid-control');
const btn_removechecked  = $('#pnl_editcompoundgrid-removechecked');
const btn_addrow = $('#pnl_editcompoundgrid-addrow');
...
export function OpenDetil(data) {
	...
	var fn_listloading = async (options) => {
		options.api = `${global.modulefullname}/compound-list`
	...
	var fn_listloaded = async (result, options) => {
		...
		var detilform = $ui.getPages().ITEMS['pnl_editcompoundform'].handler.getForm()
		...
		setTimeout(()=>{
			var checkcolumns = document.querySelectorAll('#pnl_editcompoundgrid-tbl_list .rowcheck');
			...
			var selectbutton = document.getElementById('pnl_editcompoundgrid-tbl_list-selectall-button');
			if (detilform.AllowRemoveRecord) {
...
function grd_list_rowclick(tr, ev) {
	...
	$ui.getPages().show('pnl_editcompoundform', () => {
		$ui.getPages().ITEMS['pnl_editcompoundform'].handler.open(record, trid, header_data)
	})	
}
		...
		yes: () => {
			grd_list.removechecked({
				OnRemoving : async (options) => {
					var apiurl = `${global.modulefullname}/compound-delete`
...
function btn_addrow_click() {
	$ui.getPages().show('pnl_editcompoundform', ()=>{
		$ui.getPages().ITEMS['pnl_editcompoundform'].handler.createnew(header_data)
	})	
}
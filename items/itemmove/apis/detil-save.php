...
/**
 * finact/items/itemmove/apis/detil-save.php
 *
 * ==========
 * Detil-Save
 * ==========
 * Menampilkan satu baris data/record sesuai PrimaryKey,
 * dari tabel detil itemmove (trn_itemmove)
 *
...
$API = new class extends itemmoveBase {
	...
	public function execute($data, $options) {
		$tablename = 'trn_itemmovedetil';
		$primarykey = 'itemmovedetil_id';
		$autoid = $options->autoid;
		$datastate = $data->_state;

		$userdata = $this->auth->session_get_user();

		$handlerclassname = "\\FGTA4\\apis\\itemmove_detilHandler";
		if (class_exists($handlerclassname)) {
			$hnd = new itemmove_detilHandler($data, $options);
			$hnd->caller = $this;
			$hnd->db = $this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			if ($obj->item_id=='') { $obj->item_id = '--NULL--'; }
			...
			try {
				...
				$header_table = 'trn_itemmove';
				$header_primarykey = 'itemmove_id';
				...
				$sql = \FGTA4\utils\SqlUtility::Select($tablename , [
					$primarykey
					, 'itemmovedetil_id', 'item_id', 'item_name', 'itemmovedetil_qtyprop', 'itemmovedetil_qtysend', 'itemmovedetil_qtyrecv', 'itemmove_id', '_createby', '_createdate', '_modifyby', '_modifydate', '_createby', '_createdate', '_modifyby', '_modifydate'
				], $where->sql);

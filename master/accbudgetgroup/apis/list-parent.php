...
$API = new class extends accbudgetgroupBase {
	...
	public function execute($options) {
		...
		try {
			...
			\FGTA4\utils\SqlUtility::setDefaultCriteria($options->criteria, 'accbudgetgroup_isparent', '1');
			\FGTA4\utils\SqlUtility::setDefaultCriteria($options->criteria, 'accbudgetgroup_isdisabled', '0');
			$where = \FGTA4\utils\SqlUtility::BuildCriteria(
				$options->criteria,
				[
					"search" => " A.accbudgetgroup_id LIKE CONCAT('%', :search, '%') OR A.accbudgetgroup_name LIKE CONCAT('%', :search, '%') ",
					"accbudgetgroup_isparent" => " A.accbudgetgroup_isparent = :accbudgetgroup_isparent ",
					"accbudgetgroup_isdisabled" => " A.accbudgetgroup_isdisabled = :accbudgetgroup_isdisabled ",
				]
			);
			...
			$stmt = $this->db->prepare("
					select 
					  A.accbudgetgroup_id
					, A.accbudgetgroup_id as accbudgetgroup_parent, A.accbudgetgroup_name as accbudgetgroup_parent_name
					, A.accbudgetmodel_id					  

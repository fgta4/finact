...
/**
 * finact/master/project/apis/task-save.php
 *
 * ==========
 * Detil-Save
 * ==========
...
$API = new class extends projectBase {
	...
	public function execute($data, $options) {
		...
		try {
			...
			$obj->projecttask_dtstart = (\DateTime::createFromFormat('d/m/Y',$obj->projecttask_dtstart))->format('Y-m-d');
			$obj->projecttask_dtend = (\DateTime::createFromFormat('d/m/Y',$obj->projecttask_dtend))->format('Y-m-d');
			...
			try {
				...
				$header_table = 'mst_project';
				$header_primarykey = 'project_id';
				...
				$sql = \FGTA4\utils\SqlUtility::Select($tablename , [
					$primarykey
					, 'projecttask_id', 'projecttask_name', 'projecttask_descr', 'projecttask_dtstart', 'projecttask_dtend', 'dept_id', 'project_id', '_createby', '_createdate', '_modifyby', '_modifydate', '_createby', '_createdate', '_modifyby', '_modifydate'
				], $where->sql);

...
const tbl_list = $('#pnl_edittaskgrid-tbl_list');
const txt_title = $('#pnl_edittaskgrid-title');
const pnl_control = $('.pnl_edittaskgrid-control');
const btn_removechecked  = $('#pnl_edittaskgrid-removechecked');
const btn_addrow = $('#pnl_edittaskgrid-addrow');
...
export function createnew(data, options) {
	...
	grd_list.clear();
	txt_title.html(data.project_name)
	header_data = data;
}
...
export function OpenDetil(data) {
	...
	var fn_listloading = async (options) => {
		options.api = `${global.modulefullname}/task-list`
		options.criteria['id'] = data.project_id
	}
	var fn_listloaded = async (result, options) => {
		...
		var detilform = $ui.getPages().ITEMS['pnl_edittaskform'].handler.getForm()
...
function grd_list_rowclick(tr, ev) {
	...
	$ui.getPages().show('pnl_edittaskform', () => {
		$ui.getPages().ITEMS['pnl_edittaskform'].handler.open(record, trid, header_data)
	})	
}
		...
		yes: () => {
			grd_list.removechecked({
				OnRemoving : async (options) => {
					var apiurl = `${global.modulefullname}/task-delete`
...
function btn_addrow_click() {
	$ui.getPages().show('pnl_edittaskform', ()=>{
		$ui.getPages().ITEMS['pnl_edittaskform'].handler.createnew(header_data)
	})	
}
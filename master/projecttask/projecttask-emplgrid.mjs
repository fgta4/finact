...
const tbl_list = $('#pnl_editemplgrid-tbl_list')
const txt_title = $('#pnl_editemplgrid-title')
const pnl_control = $('#pnl_editemplgrid-control')
const btn_removechecked  = $('#pnl_editemplgrid-removechecked')
const btn_addrow = $('#pnl_editemplgrid-addrow')
...
export function createnew(data, options) {
	...
	grd_list.clear()
	txt_title.html(data.projecttask_name)
	header_data = data
}
...
export function OpenDetil(data) {
	...
	var fn_listloading = async (options) => {
		options.api = `${global.modulefullname}/empl-list`
		options.criteria['id'] = data.projecttask_id
	}
	var fn_listloaded = async (result, options) => {
		...
		var detilform = $ui.getPages().ITEMS['pnl_editemplform'].handler.getForm()
...
function grd_list_rowclick(tr, ev) {
	...
	$ui.getPages().show('pnl_editemplform', () => {
		$ui.getPages().ITEMS['pnl_editemplform'].handler.open(record, trid, header_data)
	})	
}
		...
		yes: () => {
			grd_list.removechecked({
				OnRemoving : async (options) => {
					var apiurl = `${global.modulefullname}/empl-delete`
...
function btn_addrow_click() {
	$ui.getPages().show('pnl_editemplform', ()=>{
		$ui.getPages().ITEMS['pnl_editemplform'].handler.createnew(header_data)
	})	
}
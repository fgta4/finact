...
const tbl_list = $('#pnl_editcoafiltergrid-tbl_list');
const txt_title = $('#pnl_editcoafiltergrid-title');
const pnl_control = $('#pnl_editcoafiltergrid-control');
const btn_removechecked  = $('#pnl_editcoafiltergrid-removechecked');
const btn_addrow = $('#pnl_editcoafiltergrid-addrow');
...
export function createnew(data, options) {
	...
	grd_list.clear();
	txt_title.html(data.accfin_name)
	header_data = data;
}
...
export function OpenDetil(data) {
	...
	var fn_listloading = async (options) => {
		options.api = `${global.modulefullname}/coafilter-list`
		options.criteria['id'] = data.accfin_id
	}
	var fn_listloaded = async (result, options) => {
		...
		var detilform = $ui.getPages().ITEMS['pnl_editcoafilterform'].handler.getForm()
...
function grd_list_rowclick(tr, ev) {
	...
	$ui.getPages().show('pnl_editcoafilterform', () => {
		$ui.getPages().ITEMS['pnl_editcoafilterform'].handler.open(record, trid, header_data)
	})	
}
		...
		yes: () => {
			grd_list.removechecked({
				OnRemoving : async (options) => {
					var apiurl = `${global.modulefullname}/coafilter-delete`
...
function btn_addrow_click() {
	$ui.getPages().show('pnl_editcoafilterform', ()=>{
		$ui.getPages().ITEMS['pnl_editcoafilterform'].handler.createnew(header_data)
	})	
}
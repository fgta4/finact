...
import * as hnd from  './jurnalsource-jurnaltypegrid-hnd.mjs'

const tbl_list = $('#pnl_editjurnaltypegrid-tbl_list');
const txt_title = $('#pnl_editjurnaltypegrid-title');
const pnl_control = $('.pnl_editjurnaltypegrid-control');
const btn_removechecked  = $('#pnl_editjurnaltypegrid-removechecked');
const btn_addrow = $('#pnl_editjurnaltypegrid-addrow');
...
export function createnew(data, options) {
	...
	txt_title.html(data.jurnalsource_name)
...
export function OpenDetil(data) {
	...
	var fn_listloading = async (options) => {
		options.api = `${global.modulefullname}/jurnaltype-list`
		options.criteria['id'] = data.jurnalsource_id
	...
	var fn_listloaded = async (result, options) => {
		...
		var detilform = $ui.getPages().ITEMS['pnl_editjurnaltypeform'].handler.getForm()
		...
		setTimeout(()=>{
			var checkcolumns = document.querySelectorAll('#pnl_editjurnaltypegrid-tbl_list .rowcheck');
			...
			var selectbutton = document.getElementById('pnl_editjurnaltypegrid-tbl_list-selectall-button');
			if (detilform.AllowRemoveRecord) {
...
function grd_list_rowclick(tr, ev) {
	...
	$ui.getPages().show('pnl_editjurnaltypeform', () => {
		$ui.getPages().ITEMS['pnl_editjurnaltypeform'].handler.open(record, trid, header_data)
	})	
}
		...
		yes: () => {
			grd_list.removechecked({
				OnRemoving : async (options) => {
					var apiurl = `${global.modulefullname}/jurnaltype-delete`
...
function btn_addrow_click() {
	$ui.getPages().show('pnl_editjurnaltypeform', ()=>{
		$ui.getPages().ITEMS['pnl_editjurnaltypeform'].handler.createnew(header_data)
	})	
}
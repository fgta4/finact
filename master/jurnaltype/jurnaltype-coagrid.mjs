...
import * as hnd from  './jurnaltype-coagrid-hnd.mjs'

const tbl_list = $('#pnl_editcoagrid-tbl_list');
const txt_title = $('#pnl_editcoagrid-title');
const pnl_control = $('.pnl_editcoagrid-control');
const btn_removechecked  = $('#pnl_editcoagrid-removechecked');
const btn_addrow = $('#pnl_editcoagrid-addrow');
...
export function createnew(data, options) {
	...
	txt_title.html(data.jurnaltype_name)
...
export function OpenDetil(data) {
	...
	var fn_listloading = async (options) => {
		options.api = `${global.modulefullname}/coa-list`
		options.criteria['id'] = data.jurnaltype_id
	...
	var fn_listloaded = async (result, options) => {
		...
		var detilform = $ui.getPages().ITEMS['pnl_editcoaform'].handler.getForm()
		...
		setTimeout(()=>{
			var checkcolumns = document.querySelectorAll('#pnl_editcoagrid-tbl_list .rowcheck');
			...
			var selectbutton = document.getElementById('pnl_editcoagrid-tbl_list-selectall-button');
			if (detilform.AllowRemoveRecord) {
...
function grd_list_rowclick(tr, ev) {
	...
	$ui.getPages().show('pnl_editcoaform', () => {
		$ui.getPages().ITEMS['pnl_editcoaform'].handler.open(record, trid, header_data)
	})	
}
		...
		yes: () => {
			grd_list.removechecked({
				OnRemoving : async (options) => {
					var apiurl = `${global.modulefullname}/coa-delete`
...
function btn_addrow_click() {
	$ui.getPages().show('pnl_editcoaform', ()=>{
		$ui.getPages().ITEMS['pnl_editcoaform'].handler.createnew(header_data)
	})	
}
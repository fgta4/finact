...
/**
 * finact/master/taxtype/apis/save.php
 *
 * ====
 * Save
 * ====
...
$API = new class extends taxtypeBase {
	...
	public function execute($data, $options) {
		...
		try {
			...
			$obj->taxtype_id = strtoupper($obj->taxtype_id);
			$obj->taxtype_name = strtoupper($obj->taxtype_name);


			// if ($obj->billin_coa_id=='--NULL--') { unset($obj->billin_coa_id); }
			// if ($obj->billout_coa_id=='--NULL--') { unset($obj->billout_coa_id); }
			...
			try {
				...
				$sql = \FGTA4\utils\SqlUtility::Select($tablename , [
					$primarykey
					, 'taxtype_id', 'taxtype_name', 'taxtype_include', 'taxtype_descr', 'taxtype_value', 'taxmodel_id', 'billin_coa_id', 'billout_coa_id', '_createby', '_createdate', '_modifyby', '_modifydate', '_createby', '_createdate', '_modifyby', '_modifydate'
				], $where->sql);

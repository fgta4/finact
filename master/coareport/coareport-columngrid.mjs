...
const pnl_control = $('.pnl_editcolumngrid-control');
const btn_removechecked  = $('#pnl_editcolumngrid-removechecked');
const btn_addrow = $('#pnl_editcolumngrid-addrow');
...
export function OpenDetil(data) {
	...
	var fn_listloading = async (options) => {
		...
		options.criteria['id'] = data.coareport_id
...
function grd_list_rowclick(tr, ev) {
	...
	$ui.getPages().show('pnl_editcolumnform', () => {
		$ui.getPages().ITEMS['pnl_editcolumnform'].handler.open(record, trid, header_data)
	})	
}
		...
		yes: () => {
			grd_list.removechecked({
				OnRemoving : async (options) => {
					var apiurl = `${global.modulefullname}/column-delete`
...
function btn_addrow_click() {
	$ui.getPages().show('pnl_editcolumnform', ()=>{
		$ui.getPages().ITEMS['pnl_editcolumnform'].handler.createnew(header_data)
	})	
}
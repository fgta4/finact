...
/**
 * finact/master/coareport/apis/delete.php
 *
 * ======
 * Delete
 * ======
 * Menghapus satu baris data/record berdasarkan PrimaryKey
 * pada tabel header coareport (mst_coareport)
 *
...
$API = new class extends coareportBase {
	...
	public function execute($data, $options) {
		$tablename = 'mst_coareport';
		$primarykey = 'coareport_id';

		$userdata = $this->auth->session_get_user();
		$handlerclassname = "\\FGTA4\\apis\\coareport_headerHandler";
		$hnd = null;
		if (class_exists($handlerclassname)) {
			$hnd = new /*coareport*/_headerHandler($options);
			$hnd->caller = &$this;
			$hnd->db = &$this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			try {
				...
				$tabletodelete = ['mst_coareportcol'];

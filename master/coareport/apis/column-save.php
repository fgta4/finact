...
if (is_file(__DIR__ .'/data-column-handler.php')) {
	require_once __DIR__ .'/data-column-handler.php';
}
...
/**
 * finact/master/coareport/apis/column-save.php
 *
 * ==========
 * Detil-Save
 * ==========
 * Menampilkan satu baris data/record sesuai PrimaryKey,
 * dari tabel column coareport (mst_coareportcol)
 *
...
$API = new class extends coareportBase {
	...
	public function execute($data, $options) {
		$event = 'on-save';
		$tablename = 'mst_coareportcol';
		$primarykey = 'coareportcol_id';
		...
		if (class_exists($handlerclassname)) {
			$hnd = new coareport_columnHandler($data, $options);
			$hnd->caller = &$this;
			$hnd->db = &$this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			try {
				...
				$header_table = 'mst_coareport';
				$header_primarykey = 'coareport_id';
				...
				// result
				$options->criteria = [
					"coareportcol_id" => $obj->coareportcol_id
				];

				$criteriaValues = [
					"coareportcol_id" => " coareportcol_id = :coareportcol_id "
				];
				...
				$sqlFieldList = [
					'coareportcol_id' => 'A.`coareportcol_id`', 'coareportcol_name' => 'A.`coareportcol_name`', 'coareport_col' => 'A.`coareport_col`', 'coa_dk' => 'A.`coa_dk`',
					'coareport_id' => 'A.`coareport_id`', '_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`', '_modifyby' => 'A.`_modifyby`',
					'_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`', '_modifyby' => 'A.`_modifyby`', '_modifydate' => 'A.`_modifydate`'
				];
				$sqlFromTable = "mst_coareportcol A";
				$sqlWhere = $where->sql;

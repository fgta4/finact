...
class PrintForm extends WebModule {
	function __construct() {
		// $logfilepath = __LOCALDB_DIR . "/output/logfile-orderindelv.txt";
	...
	public function LoadPage() {
		...
		try {
			...
			$stmt->execute([':orderindelv_id' => $id]);
			$rows  = $stmt->fetchall(\PDO::FETCH_ASSOC);
			$row = $rows[0];

			$this->orderindelv_id = $row['orderindelv_id'];
	...
	function getSqlHeader() {
		return "
			...
			from trn_orderindelv A where orderindelv_id = :orderindelv_id
		";
	}	

...
const tbl_list = $('#pnl_edittermsgrid-tbl_list');
const txt_title = $('#pnl_edittermsgrid-title');
const pnl_control = $('#pnl_edittermsgrid-control');
const btn_removechecked  = $('#pnl_edittermsgrid-removechecked');
const btn_addrow = $('#pnl_edittermsgrid-addrow');
...
export function createnew(data, options) {
	...
	grd_list.clear();
	txt_title.html(data.orderin_descr)
	header_data = data;
}
...
export function OpenDetil(data) {
	...
	var fn_listloading = async (options) => {
		options.api = `${global.modulefullname}/terms-list`
		options.criteria['id'] = data.orderin_id
	}
	var fn_listloaded = async (result, options) => {
		...
		var detilform = $ui.getPages().ITEMS['pnl_edittermsform'].handler.getForm()
...
function grd_list_rowclick(tr, ev) {
	...
	$ui.getPages().show('pnl_edittermsform', () => {
		$ui.getPages().ITEMS['pnl_edittermsform'].handler.open(record, trid, header_data)
	})	
}
		...
		yes: () => {
			grd_list.removechecked({
				OnRemoving : async (options) => {
					var apiurl = `${global.modulefullname}/terms-delete`
...
function btn_addrow_click() {
	$ui.getPages().show('pnl_edittermsform', ()=>{
		$ui.getPages().ITEMS['pnl_edittermsform'].handler.createnew(header_data)
	})	
}
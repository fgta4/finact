...
class PrintForm extends WebModule {
	function __construct() {
		// $logfilepath = __LOCALDB_DIR . "/output/logfile-orderin.txt";
	...
	public function LoadPage() {
		...
		try {
			...
			$stmt->execute([':orderin_id' => $id]);
			$rows  = $stmt->fetchall(\PDO::FETCH_ASSOC);
			$row = $rows[0];

			$this->orderin_id = $row['orderin_id'];
	...
	function getSqlHeader() {
		return "
			...
			from trn_orderin A where orderin_id = :orderin_id
		";
	}	

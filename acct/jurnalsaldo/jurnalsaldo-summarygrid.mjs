...
const tbl_list = $('#pnl_editsummarygrid-tbl_list');
const txt_title = $('#pnl_editsummarygrid-title');
const pnl_control = $('.pnl_editsummarygrid-control');
const btn_removechecked  = $('#pnl_editsummarygrid-removechecked');
const btn_addrow = $('#pnl_editsummarygrid-addrow');
...
export function OpenDetil(data) {
	...
	var fn_listloaded = async (result, options) => {
		...
		var detilform = $ui.getPages().ITEMS['pnl_editsummaryform'].handler.getForm()
		...
		setTimeout(()=>{
			var checkcolumns = document.querySelectorAll('#pnl_editsummarygrid-tbl_list .rowcheck');
			...
			var selectbutton = document.getElementById('pnl_editsummarygrid-tbl_list-selectall-button');
			if (detilform.AllowRemoveRecord) {
...
function grd_list_rowclick(tr, ev) {
	...
	$ui.getPages().show('pnl_editsummaryform', () => {
		$ui.getPages().ITEMS['pnl_editsummaryform'].handler.open(record, trid, header_data)
	})	
}
		...
		yes: () => {
			grd_list.removechecked({
				OnRemoving : async (options) => {
					var apiurl = `${global.modulefullname}/summary-delete`
...
function btn_addrow_click() {
	$ui.getPages().show('pnl_editsummaryform', ()=>{
		$ui.getPages().ITEMS['pnl_editsummaryform'].handler.createnew(header_data)
	})	
}
...
$API = new class extends jurnalsaldoBase {
	...
	public function execute($data, $options) {
		...
		$handlerclassname = "\\FGTA4\\apis\\jurnalsaldo_headerHandler";
		$hnd = null;
		if (class_exists($handlerclassname)) {
			$hnd = new jurnalsaldo_headerHandler($options);
			$hnd->caller = &$this;
			$hnd->db = &$this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			$obj->periodemo_dtstart = (\DateTime::createFromFormat('d/m/Y',$obj->periodemo_dtstart))->format('Y-m-d');
			$obj->periodemo_dtend = (\DateTime::createFromFormat('d/m/Y',$obj->periodemo_dtend))->format('Y-m-d');
			$obj->periodemo_closedate = (\DateTime::createFromFormat('d/m/Y',$obj->periodemo_closedate))->format('Y-m-d');

			$obj->periodemo_id = strtoupper($obj->periodemo_id);
			$obj->periodemo_name = strtoupper($obj->periodemo_name);


			if ($obj->periodemo_closeby=='') { $obj->periodemo_closeby = '--NULL--'; }
			if ($obj->periodemo_closedate=='') { $obj->periodemo_closedate = '--NULL--'; }
			...
			try {
				...
				$sqlFieldList = [
					'periodemo_id' => 'A.`periodemo_id`', 'periodemo_name' => 'A.`periodemo_name`', 'periodemo_year' => 'A.`periodemo_year`', 'periodemo_month' => 'A.`periodemo_month`',
					'periodemo_dtstart' => 'A.`periodemo_dtstart`', 'periodemo_dtend' => 'A.`periodemo_dtend`', 'periodemo_isclosed' => 'A.`periodemo_isclosed`', 'periodemo_closeby' => 'A.`periodemo_closeby`',
					'periodemo_closedate' => 'A.`periodemo_closedate`', '_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`', '_modifyby' => 'A.`_modifyby`',

...
const tbl_list = $('#pnl_editsaldogrid-tbl_list');
const txt_title = $('#pnl_editsaldogrid-title');
const pnl_control = $('.pnl_editsaldogrid-control');
const btn_removechecked  = $('#pnl_editsaldogrid-removechecked');
const btn_addrow = $('#pnl_editsaldogrid-addrow');
...
export function createnew(data, options) {
	...
	txt_title.html(data.periodemo_name)
...
export function OpenDetil(data) {
	...
	var fn_listloading = async (options) => {
		options.api = `${global.modulefullname}/saldo-list`
		options.criteria['id'] = data.periodemo_id
	...
	var fn_listloaded = async (result, options) => {
		...
		var detilform = $ui.getPages().ITEMS['pnl_editsaldoform'].handler.getForm()
		...
		setTimeout(()=>{
			var checkcolumns = document.querySelectorAll('#pnl_editsaldogrid-tbl_list .rowcheck');
			...
			var selectbutton = document.getElementById('pnl_editsaldogrid-tbl_list-selectall-button');
			if (detilform.AllowRemoveRecord) {
...
function grd_list_rowclick(tr, ev) {
	...
	$ui.getPages().show('pnl_editsaldoform', () => {
		$ui.getPages().ITEMS['pnl_editsaldoform'].handler.open(record, trid, header_data)
	})	
}
		...
		yes: () => {
			grd_list.removechecked({
				OnRemoving : async (options) => {
					var apiurl = `${global.modulefullname}/saldo-delete`
...
function btn_addrow_click() {
	$ui.getPages().show('pnl_editsaldoform', ()=>{
		$ui.getPages().ITEMS['pnl_editsaldoform'].handler.createnew(header_data)
	})	
}
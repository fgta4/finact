...
async function report_prepare(param) {
	...
	if (parent.window.rpt!==undefined) {
		if (typeof parent.window.rpt.prepareData === 'function') {
			...
			// bedasarkan report properties hasil dari api prepare data
			if (typeof parent.window.getReportProperties === 'function') {
				var report_properties = parent.window.getReportProperties();
				if (report_properties.prevColTitle!=null) {
					document.getElementById('obj_reporttable-prevcaption-web').innerHTML = report_properties.prevColTitle;
				}
				if (report_properties.currColTitle!=null) {
					document.getElementById('obj_reporttable-currcaption-web').innerHTML = report_properties.currColTitle;
				}
			}

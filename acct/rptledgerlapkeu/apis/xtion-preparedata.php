...
$API = new class extends rptledgersummaryBase {
	...
	public function execute($param) {
		...
		try {
			...
			$prevColTitle = 's/d Bulan Lalu';
			$currColTitle = 'Bulan Ini';
			if ($span=='ytd') {
				$prevColTitle = 'Tahun Lalu';
				$currColTitle = 'Tahun Ini';
			}

			/** Pilih stored procedure yang akan digunakan */
			$spreportname = 'neraca';
			if ($report=='LR') {
				$spreportname = 'labarugi';
			}

			$sql = "CALL ledger_".$spreportname."_idr_all(:span, :dt, :cacheid)";
			if ($sp=='perunit') {
				$sql = "CALL ledger_".$spreportname."_idr_perunit(:unit_id, :span, :dt, :cacheid)";
				$sqlparam[':unit_id'] = $filtervalue;
			} else if ($sp=='perdept') {
				$sql = "CALL ledger_".$spreportname."_idr_perdept(:dept_id, :span, :dt, :cacheid)";
			...
			return (object)[
				...
				'report_properties' => (object)[
					'prevColTitle' => $prevColTitle,
					'currColTitle' => $currColTitle
				]					
			];
		} catch (\Exception $ex) {
			throw $ex;

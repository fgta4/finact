...
const cbo_coa_id = $('#pnl_report-cbo_coa_id');
...
export async function init(opt) {
	...
	cbo_coa_id.name = 'pnl_report-cbo_coa_id'	
	cbo_coa_id.comp = new fgta4slideselect(cbo_coa_id, {
		...
		OnCreated: () => {
			cbo_coa_id.combo('setValue', '--NULL--');
...
function cbo_reporttype_selected(record) {
	...
	if (optiondata.coaskip===true) {
		cbo_coa_id.combo('disable');
		cbo_coa_id.combo('setValue', '--NULL--');
		cbo_coa_id.combo('setText', '--PILIH--');
	} else {
		cbo_coa_id.combo('enable');
	}
...
function btn_load_click() {
	...
	var coa_id = cbo_coa_id.combo('getValue');
	var coa_name = cbo_coa_id.combo('getText');
	var reportmodule = window.global.modulefullname + '/rptledgercoa.xprint?renderto=printtemplate.phtml&format=print-format-01-a4-potrait';
	var optiondata = cbo_reporttype.optiondata;


	if (optiondata.coaskip!==true) {
		if (coa_id=='--NULL--' || coa_id=='' || coa_id==null) {
			$ui.ShowMessage('[WARNING] COA belum dipilih');
			return;
		}
	}

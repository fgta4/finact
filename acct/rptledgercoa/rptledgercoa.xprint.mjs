...
function report_format_row(rpt, row, cellclass) {
	...
	if (row.jurnal_id==null) {
		row.jurnal_id = '&nbsp;';
	}

	cellclass += ' ' + row.cssclass;

	if (row.cssclass=='coasum-coaname') {
		saldoawal_idr = '&nbsp;';
		debet_idr = '&nbsp;';
		kredit_idr = '&nbsp;';
		saldoakhir_idr = '&nbsp;';
	...
	var excelcelformat = "";
	if (row.cssclass=='coasum-coaname' || row.cssclass=='coasum-saldoawal' || row.cssclass=='coasum-saldoakhir') {
		excelcelformat = `data-f-bold="true"`;
	}


	/* 
		for formating tabletoexcel, cek:
		https://github.com/linways/table-to-excel
	*/		
	tr.innerHTML = `
		<td class="${cellclass} col-no"><div class="cell-row-breaker">${row.cache_rownumber}</div></td>
		<td class="${cellclass} col-coa" ${excelstyle_weight}>${row.jurnal_id}</td>
		<td class="${cellclass} col-descr" style="${style_descr}" ${excelcelformat}>${row.jurnaldetil_descr}</td>
		<td class="${cellclass} col-amount" style="${style_amount}" ${excelcelformat} data-t="n" data-a-h="right">${saldoawal_idr}</td>
		<td class="${cellclass} col-amount" style="${style_amount}" ${excelcelformat} data-t="n" data-a-h="right">${debet_idr}</td>
		<td class="${cellclass} col-amount" style="${style_amount}" ${excelcelformat} data-t="n" data-a-h="right">${kredit_idr}</td>
		<td class="${cellclass} col-amount" style="${style_amount}" ${excelcelformat} data-t="n" data-a-h="right">${saldoakhir_idr}</td>

		<!-- hanya muncul di spreadsheet -->
		<td class="col-hidden" data-a-h="center">${row.jurnaldetil_id==null?'':row.jurnaldetil_id}</td>
		<td class="col-hidden" data-a-h="center">${row.jurnal_date==null?'':row.jurnal_date}</td>
		<td class="col-hidden">${row.coa_id}</td>
		<td class="col-hidden">${row.coa_name==null?'':row.coa_name}</td>
		<td class="col-hidden">${row.unit_name==null?'':row.unit_name}</td>
		<td class="col-hidden">${row.dept_name==null?'':row.dept_name}</td>
		<td class="col-hidden">${row.partner_name==null?'':row.partner_name}</td>
		<td class="col-hidden">${row.project_name==null?'':row.project_name}</td>

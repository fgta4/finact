...
$MODULE = new class extends WebModule {
	...
	public function LoadPage(object $param) {
		...
		try {
			...
			$this->reportsubtitle = $param->optiondata->rptsubtitle;
			if ($param->optiondata->filter!=null) {
				$this->reportsubtitle = $param->optiondata->rptsubtitle . ' (' . $param->filtervalue . ')';
			}

			$this->tableheadertype = '';
			if ($param->optiondata->sp=='det') {
				$this->tableheadertype = 'det';
			} else if ($param->optiondata->sp=='coa') {
				$this->tableheadertype = 'coa';
			} 

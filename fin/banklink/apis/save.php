...
/**
 * finact/fin/banklink/apis/save.php
 *
 * ====
 * Save
 * ====
 * Menampilkan satu baris data/record sesuai PrimaryKey,
 * dari tabel header banklink (trn_bankbook)
 *
...
$API = new class extends banklinkBase {
	...
	public function execute($data, $options) {
		...
		try {
			...
			$obj->bankbook_date = (\DateTime::createFromFormat('d/m/Y',$obj->bankbook_date))->format('Y-m-d');
			...
			try {
				...
				$sql = \FGTA4\utils\SqlUtility::Select($tablename , [
					$primarykey
					, 'bankbook_id', 'bankbook_date', 'periodemo_id', 'bankrekening_id', 'curr_id', '_createby', '_createdate', '_modifyby', '_modifydate', '_createby', '_createdate', '_modifyby', '_modifydate'
				], $where->sql);
				...
				'bankbook_date' => date("d/m/Y", strtotime($row['bankbook_date'])),

...
const tbl_list = $('#pnl_edittaxgrid-tbl_list');
const txt_title = $('#pnl_edittaxgrid-title');
const pnl_control = $('#pnl_edittaxgrid-control');
const btn_removechecked  = $('#pnl_edittaxgrid-removechecked');
const btn_addrow = $('#pnl_edittaxgrid-addrow');
...
export function createnew(data, options) {
	...
	grd_list.clear();
	txt_title.html(data.billout_descr)
	header_data = data;
}
...
export function OpenDetil(data) {
	...
	var fn_listloading = async (options) => {
		options.api = `${global.modulefullname}/tax-list`
		options.criteria['id'] = data.billout_id
	}
	var fn_listloaded = async (result, options) => {
		...
		var detilform = $ui.getPages().ITEMS['pnl_edittaxform'].handler.getForm()
...
function grd_list_rowclick(tr, ev) {
	...
	$ui.getPages().show('pnl_edittaxform', () => {
		$ui.getPages().ITEMS['pnl_edittaxform'].handler.open(record, trid, header_data)
	})	
}
		...
		yes: () => {
			grd_list.removechecked({
				OnRemoving : async (options) => {
					var apiurl = `${global.modulefullname}/tax-delete`
...
function btn_addrow_click() {
	$ui.getPages().show('pnl_edittaxform', ()=>{
		$ui.getPages().ITEMS['pnl_edittaxform'].handler.createnew(header_data)
	})	
}
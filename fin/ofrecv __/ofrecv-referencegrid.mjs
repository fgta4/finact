...
const tbl_list = $('#pnl_editreferencegrid-tbl_list');
const txt_title = $('#pnl_editreferencegrid-title');
const pnl_control = $('#pnl_editreferencegrid-control');
const btn_removechecked  = $('#pnl_editreferencegrid-removechecked');
const btn_addrow = $('#pnl_editreferencegrid-addrow');
...
export function OpenDetil(data) {
	...
	var fn_listloading = async (options) => {
		options.api = `${global.modulefullname}/reference-list`
	...
	var fn_listloaded = async (result, options) => {
		...
		var detilform = $ui.getPages().ITEMS['pnl_editreferenceform'].handler.getForm()
...
function grd_list_rowclick(tr, ev) {
	...
	$ui.getPages().show('pnl_editreferenceform', () => {
		$ui.getPages().ITEMS['pnl_editreferenceform'].handler.open(record, trid, header_data)
	})	
}
		...
		yes: () => {
			grd_list.removechecked({
				OnRemoving : async (options) => {
					var apiurl = `${global.modulefullname}/reference-delete`
...
function btn_addrow_click() {
	$ui.getPages().show('pnl_editreferenceform', ()=>{
		$ui.getPages().ITEMS['pnl_editreferenceform'].handler.createnew(header_data)
	})	
}
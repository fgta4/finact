...
const tbl_list = $('#pnl_editrecvgrid-tbl_list');
const txt_title = $('#pnl_editrecvgrid-title');
const pnl_control = $('#pnl_editrecvgrid-control');
const btn_removechecked  = $('#pnl_editrecvgrid-removechecked');
const btn_addrow = $('#pnl_editrecvgrid-addrow');
...
export function OpenDetil(data) {
	...
	var fn_listloading = async (options) => {
		options.api = `${global.modulefullname}/recv-list`
		options.criteria['id'] = data.jurnal_id
		options.criteria['jurnaldetil_kredit'] = 1
	}
	var fn_listloaded = async (result, options) => {
		...
		var detilform = $ui.getPages().ITEMS['pnl_editrecvform'].handler.getForm()
...
function grd_list_rowclick(tr, ev) {
	...
	$ui.getPages().show('pnl_editrecvform', () => {
		$ui.getPages().ITEMS['pnl_editrecvform'].handler.open(record, trid, header_data)
	})	
}
		...
		yes: () => {
			grd_list.removechecked({
				OnRemoving : async (options) => {
					var apiurl = `${global.modulefullname}/recv-delete`
...
function btn_addrow_click() {
	$ui.getPages().show('pnl_editrecvform', ()=>{
		$ui.getPages().ITEMS['pnl_editrecvform'].handler.createnew(header_data)
	})	
}
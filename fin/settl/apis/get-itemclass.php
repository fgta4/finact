	...
	public function execute($options) {
		...
		try {
			...
			$where = \FGTA4\utils\SqlUtility::BuildCriteria(
				$options->criteria,
				[
					"search" => " A.itemclass_id LIKE CONCAT('%', :search, '%') OR A.inquirydetil_descr LIKE CONCAT('%', :search, '%') ",
			...
			$stmt = $this->db->prepare("
					select 
						A.itemclass_id,
						A.inquirydetil_descr,
						A.inquirydetil_qty,
						A.inquirydetil_days,
						A.inquirydetil_task,
						A.inquirydetil_estrate,
						A.inquirydetil_estvalue,
						A.coa_id,
						A.accbudget_id 
					from 
						trn_inquirydetil A

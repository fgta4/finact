...
/**
 * finact/fin/ofrecv/apis/detil-list.php
 *
 * ==============
 * Detil-DataList
 * ==============
 * Menampilkan data-data pada tabel detil ofrecv (trn_tjurnalor)
...
$API = new class extends ofrecvBase {
	...
	public function execute($options) {
		...
		try {
			...
			$stmt = $this->db->prepare("select count(*) as n from trn_tjurnalordetil A" . $where->sql);
			...
			$stmt = $this->db->prepare("
				select 
				A.jurnaldetil_id, A.partner_id, A.jurnaldetil_descr, A.jurnaldetil_valfrg, A.curr_id, A.jurnaldetil_valfrgrate, A.jurnaldetil_validr, A.coa_id, A.dept_id, A.jurnaldetil_id_ref, A.jurnal_id, A._createby, A._createdate, A._modifyby, A._modifydate 
				from trn_tjurnalordetil A
			" . $where->sql . $limit);
			$stmt->execute($where->params);

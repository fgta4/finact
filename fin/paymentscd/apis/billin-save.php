...
/**
 * finact/fin/paymentscd/apis/billin-save.php
 *
 * ==========
 * Detil-Save
 * ==========
...
$API = new class extends paymentscdBase {
	...
	public function execute($data, $options) {
		...
		try {
			...
			$obj->billinpaym_date = (\DateTime::createFromFormat('d/m/Y',$obj->billinpaym_date))->format('Y-m-d');
			$obj->billinpaym_datescd = (\DateTime::createFromFormat('d/m/Y',$obj->billinpaym_datescd))->format('Y-m-d');
			...
			try {
				...
				$header_table = 'trn_paymentscd';
				$header_primarykey = 'paymentscd_id';
				...
				$sql = \FGTA4\utils\SqlUtility::Select($tablename , [
					...
					, 'billin_id', 'paymentscd_id', '_createby', '_createdate', '_modifyby', '_modifydate', '_createby', '_createdate', '_modifyby', '_modifydate'
				], $where->sql);
				...
				'billinpaym_date' => date("d/m/Y", strtotime($row['billinpaym_date'])),
				'billinpaym_datescd' => date("d/m/Y", strtotime($row['billinpaym_datescd'])),

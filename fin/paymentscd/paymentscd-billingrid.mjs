...
const tbl_list = $('#pnl_editbillingrid-tbl_list');
const txt_title = $('#pnl_editbillingrid-title');
const pnl_control = $('.pnl_editbillingrid-control');
const btn_removechecked  = $('.pnl_editbillingrid-removechecked');
const btn_addrow = $('.pnl_editbillingrid-addrow');
const btn_addrowmulti = $('.pnl_editbillingrid-addrow-multi');
...
export function createnew(data, options) {
	...
	grd_list.clear();
	txt_title.html(data.paymentscd_descr)
	header_data = data;
}
...
export function OpenDetil(data) {
	...
	var fn_listloading = async (options) => {
		options.api = `${global.modulefullname}/billin-list`
		options.criteria['id'] = data.paymentscd_id
	}
	var fn_listloaded = async (result, options) => {
		...
		var detilform = $ui.getPages().ITEMS['pnl_editbillinform'].handler.getForm()
...
function grd_list_rowclick(tr, ev) {
	...
	$ui.getPages().show('pnl_editbillinform', () => {
		$ui.getPages().ITEMS['pnl_editbillinform'].handler.open(record, trid, header_data)
	})	
}
		...
		yes: () => {
			grd_list.removechecked({
				OnRemoving : async (options) => {
					var apiurl = `${global.modulefullname}/billin-delete`
...
function btn_addrow_click() {
	$ui.getPages().show('pnl_editbillinform', ()=>{
		$ui.getPages().ITEMS['pnl_editbillinform'].handler.createnew(header_data)
	})	
}

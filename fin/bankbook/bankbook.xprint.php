...
class PrintForm extends WebModule {
	function __construct() {
		// $logfilepath = __LOCALDB_DIR . "/output/logfile-bankbook.txt";
	...
	public function LoadPage() {
		...
		try {
			...
			$stmt->execute([':bankbook_id' => $id]);
			$rows  = $stmt->fetchall(\PDO::FETCH_ASSOC);
			$row = $rows[0];

			$this->bankbook_id = $row['bankbook_id'];
	...
	function getSqlHeader() {
		return "
			...
			from trn_bankbook A where bankbook_id = :bankbook_id
		";
	}	

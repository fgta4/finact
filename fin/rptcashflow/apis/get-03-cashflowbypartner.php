...
$API = new class extends reportBase {
	...
	public function execute($options) {
		...
		try {
			...
			$cacheid = \uniqid();
			$partner_id = $options->criteria->partner_id;


			// // Geneate Summary Data into Cache
			$sql = "
				CALL cashflow_summary_idr_all(:date, :cacheid, 'bypartner', NULL, :partner_id, NULL)
			";
			$stmt = $this->db->prepare($sql);
			$stmt->execute([':date'=>$date, ':cacheid'=>$cacheid, ':partner_id'=>$partner_id]);		

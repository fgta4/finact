...
$API = new class extends reportBase {
	...
	public function execute($options) {
		...
		try {
			...
			$cacheid = \uniqid();
			$project_id = $options->criteria->project_id;


			// // Geneate Summary Data into Cache
			$sql = "
				CALL cashflow_summary_idr_all(:date, :cacheid, 'byproject', NULL, NULL, :project_id)
			";
			$stmt = $this->db->prepare($sql);
			$stmt->execute([':date'=>$date, ':cacheid'=>$cacheid, ':project_id'=>$project_id]);		

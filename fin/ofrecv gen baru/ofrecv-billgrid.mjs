...
const tbl_list = $('#pnl_editbillgrid-tbl_list');
const txt_title = $('#pnl_editbillgrid-title');
const pnl_control = $('#pnl_editbillgrid-control');
const btn_removechecked  = $('#pnl_editbillgrid-removechecked');
const btn_addrow = $('#pnl_editbillgrid-addrow');
...
export function OpenDetil(data) {
	...
	var fn_listloading = async (options) => {
		options.api = `${global.modulefullname}/bill-list`
	...
	var fn_listloaded = async (result, options) => {
		...
		var detilform = $ui.getPages().ITEMS['pnl_editbillform'].handler.getForm()
...
function grd_list_rowclick(tr, ev) {
	...
	$ui.getPages().show('pnl_editbillform', () => {
		$ui.getPages().ITEMS['pnl_editbillform'].handler.open(record, trid, header_data)
	})	
}
		...
		yes: () => {
			grd_list.removechecked({
				OnRemoving : async (options) => {
					var apiurl = `${global.modulefullname}/bill-delete`
...
function btn_addrow_click() {
	$ui.getPages().show('pnl_editbillform', ()=>{
		$ui.getPages().ITEMS['pnl_editbillform'].handler.createnew(header_data)
	})	
}
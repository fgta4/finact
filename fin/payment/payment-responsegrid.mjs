...
const tbl_list = $('#pnl_editresponsegrid-tbl_list');
const txt_title = $('#pnl_editresponsegrid-title');
const pnl_control = $('#pnl_editresponsegrid-control');
const btn_removechecked  = $('#pnl_editresponsegrid-removechecked');
const btn_addrow = $('#pnl_editresponsegrid-addrow');
...
export function OpenDetil(data) {
	...
	var fn_listloading = async (options) => {
		options.api = `${global.modulefullname}/response-list`
	...
	var fn_listloaded = async (result, options) => {
		...
		var detilform = $ui.getPages().ITEMS['pnl_editresponseform'].handler.getForm()
...
function grd_list_rowclick(tr, ev) {
	...
	$ui.getPages().show('pnl_editresponseform', () => {
		$ui.getPages().ITEMS['pnl_editresponseform'].handler.open(record, trid, header_data)
	})	
}
		...
		yes: () => {
			grd_list.removechecked({
				OnRemoving : async (options) => {
					var apiurl = `${global.modulefullname}/response-delete`
...
function btn_addrow_click() {
	$ui.getPages().show('pnl_editresponseform', ()=>{
		$ui.getPages().ITEMS['pnl_editresponseform'].handler.createnew(header_data)
	})	
}
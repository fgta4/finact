...
/**
 * finact/fin/payment/apis/delete.php
 *
 * ======
 * Delete
 * ======
 * Menghapus satu baris data/record berdasarkan PrimaryKey
 * pada tabel header payment (trn_jurnal)
 *
...
$API = new class extends paymentBase {
	...
	public function execute($data, $options) {
		...
		try {
			...
			try {
				// Delete Detil
				$detils = ['trn_jurextpvdetil', 'trn_jurextpv', 'trn_jurnaldetil'];
				foreach ($detils as $tabledetilname) {
					$cmd = \FGTA4\utils\SqlUtility::CreateSQLDelete($tabledetilname, $key);

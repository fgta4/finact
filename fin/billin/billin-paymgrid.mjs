...
const tbl_list = $('#pnl_editpaymgrid-tbl_list');
const txt_title = $('#pnl_editpaymgrid-title');
const pnl_control = $('#pnl_editpaymgrid-control');
const btn_removechecked  = $('#pnl_editpaymgrid-removechecked');
const btn_addrow = $('#pnl_editpaymgrid-addrow');
...
export function OpenDetil(data) {
	...
	var fn_listloading = async (options) => {
		options.api = `${global.modulefullname}/paym-list`
	...
	var fn_listloaded = async (result, options) => {
		...
		var detilform = $ui.getPages().ITEMS['pnl_editpaymform'].handler.getForm()
...
function grd_list_rowclick(tr, ev) {
	...
	$ui.getPages().show('pnl_editpaymform', () => {
		$ui.getPages().ITEMS['pnl_editpaymform'].handler.open(record, trid, header_data)
	})	
}
		...
		yes: () => {
			grd_list.removechecked({
				OnRemoving : async (options) => {
					var apiurl = `${global.modulefullname}/paym-delete`
...
function btn_addrow_click() {
	$ui.getPages().show('pnl_editpaymform', ()=>{
		$ui.getPages().ITEMS['pnl_editpaymform'].handler.createnew(header_data)
	})	
}
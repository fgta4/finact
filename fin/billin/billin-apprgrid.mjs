...
const tbl_list = $('#pnl_editapprgrid-tbl_list');
const txt_title = $('#pnl_editapprgrid-title');
const pnl_control = $('#pnl_editapprgrid-control');
const btn_removechecked  = $('#pnl_editapprgrid-removechecked');
const btn_addrow = $('#pnl_editapprgrid-addrow');
...
export function OpenDetil(data) {
	...
	var fn_listloading = async (options) => {
		options.api = `${global.modulefullname}/appr-list`
	...
	var fn_listloaded = async (result, options) => {
		...
		var detilform = $ui.getPages().ITEMS['pnl_editapprform'].handler.getForm()
...
function grd_list_rowclick(tr, ev) {
	...
	$ui.getPages().show('pnl_editapprform', () => {
		$ui.getPages().ITEMS['pnl_editapprform'].handler.open(record, trid, header_data)
	})	
}
		...
		yes: () => {
			grd_list.removechecked({
				OnRemoving : async (options) => {
					var apiurl = `${global.modulefullname}/appr-delete`
...
function btn_addrow_click() {
	$ui.getPages().show('pnl_editapprform', ()=>{
		$ui.getPages().ITEMS['pnl_editapprform'].handler.createnew(header_data)
	})	
}
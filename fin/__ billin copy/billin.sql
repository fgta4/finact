CREATE TABLE `trn_billin` (
	...
	`purchorder_id` varchar(30)  , 
	`purchrecv_id` varchar(30)  , 
...
ALTER TABLE `trn_billin` ADD KEY `purchorder_id` (`purchorder_id`);
ALTER TABLE `trn_billin` ADD KEY `purchrecv_id` (`purchrecv_id`);
ALTER TABLE `trn_billin` ADD KEY `dept_id` (`dept_id`);
...
ALTER TABLE `trn_billin` ADD CONSTRAINT `fk_trn_billin_trn_purchorder` FOREIGN KEY (`purchorder_id`) REFERENCES `trn_purchorder` (`purchorder_id`);
ALTER TABLE `trn_billin` ADD CONSTRAINT `fk_trn_billin_trn_purchrecv` FOREIGN KEY (`purchrecv_id`) REFERENCES `trn_purchrecv` (`purchrecv_id`);
ALTER TABLE `trn_billin` ADD CONSTRAINT `fk_trn_billin_mst_dept` FOREIGN KEY (`dept_id`) REFERENCES `mst_dept` (`dept_id`);
ALTER TABLE `trn_billin` ADD CONSTRAINT `fk_trn_billin_mst_dept_2` FOREIGN KEY (`process_dept_id`) REFERENCES `mst_dept` (`dept_id`);

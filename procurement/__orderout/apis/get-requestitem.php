	...
	public function execute($options) {
		...
		try {
			...
			$where = \FGTA4\utils\SqlUtility::BuildCriteria(
				...
				[
					"id" => " A.request_id = :id",
					"search" => " A.requestitem_descr LIKE CONCAT('%', :search, '%') ",
			...
			$stmt = $this->db->prepare("select count(*) as n from trn_requestitem A" . $where->sql);
			...
			$stmt = $this->db->prepare("
				select 
				A.requestitem_id, A.itemasset_id, A.item_id, A.itemstock_id, A.partner_id, A.itemclass_id, A.requestitem_descr, A.requestitem_qty, A.requestitem_days, A.requestitem_task, A.requestitem_estrate, A.requestitem_estvalue, A.projbudgetdet_id, A.requestitem_isunbudget, A.requestitem_isallowoverbudget, A.requestitem_value, A.requestitem_budgetavailable, A.requestitem_qtyavailable, A.accbudget_id, A.coa_id, A.inquiry_id, A.inquirydetil_id, A.request_id, A._createby, A._createdate, A._modifyby, A._modifydate 
				from trn_requestitem A
			" . $where->sql . $limit);
			$stmt->execute($where->params);

...
const tbl_list = $('#pnl_editregistergrid-tbl_list');
const txt_title = $('#pnl_editregistergrid-title');
const pnl_control = $('#pnl_editregistergrid-control');
const btn_removechecked  = $('#pnl_editregistergrid-removechecked');
const btn_addrow = $('#pnl_editregistergrid-addrow');
const btn_registerall = $('.pnl_editregistergrid-registerall');
...
export async function init(opt) {
	...
	btn_registerall.linkbutton({  onClick: () => { btn_registerall_click() }  })
...
export function createnew(data, options) {
	...
	grd_list.clear();
	txt_title.html(data.recv_descr)
	header_data = data;
}
...
export function OpenDetil(data) {
	...
	var fn_listloading = async (options) => {
		options.api = `${global.modulefullname}/register-list`
		options.criteria['id'] = data.recv_id
	}
	var fn_listloaded = async (result, options) => {
		// console.log(result)
		var detilform = $ui.getPages().ITEMS['pnl_editregisterform'].handler.getForm()
...
function grd_list_rowclick(tr, ev) {
	...
	$ui.getPages().show('pnl_editregisterform', () => {
		$ui.getPages().ITEMS['pnl_editregisterform'].handler.open(record, trid, header_data)
	})	
}
		...
		yes: () => {
			grd_list.removechecked({
				OnRemoving : async (options) => {
					var apiurl = `${global.modulefullname}/register-delete`
...
function btn_addrow_click() {
	$ui.getPages().show('pnl_editregisterform', ()=>{
		$ui.getPages().ITEMS['pnl_editregisterform'].handler.createnew(header_data)
	})	
}

async function btn_registerall_click() {
	var data = {
		recv_id: header_data.recv_id
	};
	var apiurl = `${global.modulefullname}/register-allitem`

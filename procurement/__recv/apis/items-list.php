...
/**
 * finact/procurement/recv/apis/items-list.php
 *
 * ==============
 * Detil-DataList
 * ==============
 * Menampilkan data-data pada tabel items recv (trn_recv)
...
$API = new class extends recvBase {
	...
	public function execute($options) {
		...
		try {
			...
			$stmt = $this->db->prepare("select count(*) as n from trn_recvitem A" . $where->sql);
			...
			$stmt = $this->db->prepare("
				select 
				A.recvitem_id, A.itemasset_id, A.item_id, A.itemstock_id, A.partner_id, A.itemclass_id, A.recvitem_descr, A.recvitem_qty, A.recvitem_value, A.curr_id, A.recvitem_idr, A.accbudget_id, A.coa_id, A.orderout_id, A.orderoutitem_id, A.request_id, A.requestitem_id, A.inquiry_id, A.inquiryitem_id, A.recv_id, A._createby, A._createdate, A._modifyby, A._modifydate 
				from trn_recvitem A
			" . $where->sql . $limit);
			$stmt->execute($where->params);

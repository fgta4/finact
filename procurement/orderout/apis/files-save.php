...
/**
 * finact/procurement/orderout/apis/files-save.php
 *
 * ==========
 * Detil-Save
 * ==========
 * Menampilkan satu baris data/record sesuai PrimaryKey,
 * dari tabel files orderout (trn_orderoutfiles)
 *
...
$API = new class extends orderoutBase {
	...
	public function execute($data, $options, $files) {
		$event = 'on-save';
		$tablename = 'trn_orderoutfiles';
		$primarykey = 'orderoutfiles_id';
		$autoid = $options->autoid;
		$datastate = $data->_state;

		$userdata = $this->auth->session_get_user();

		$handlerclassname = "\\FGTA4\\apis\\orderout_filesHandler";
		if (class_exists($handlerclassname)) {
			$hnd = new orderout_filesHandler($data, $options);
			$hnd->caller = &$this;
			$hnd->db = &$this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			$sqlUpdateField  = [
					'orderoutfiles_id', 'doctype_id', 'orderoutfiles_descr', 'orderoutfiles_order',
					'orderoutfiles_file', 'orderout_id'
			];
			...
			try {
				...
				if (property_exists($files, $fieldname)) {
					...
					$objfile->orderoutfiles_file = $rev;
				...
				// result
				$options->criteria = [
					"orderoutfiles_id" => $obj->orderoutfiles_id
				];

				$criteriaValues = [
					"orderoutfiles_id" => " orderoutfiles_id = :orderoutfiles_id "
				];
				...
				$sqlFieldList = [
					'orderoutfiles_id' => 'A.`orderoutfiles_id`', 'doctype_id' => 'A.`doctype_id`', 'orderoutfiles_descr' => 'A.`orderoutfiles_descr`', 'orderoutfiles_order' => 'A.`orderoutfiles_order`',
					'orderoutfiles_file' => 'A.`orderoutfiles_file`', 'orderout_id' => 'A.`orderout_id`', '_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`',
					'_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`', '_modifyby' => 'A.`_modifyby`', '_modifydate' => 'A.`_modifydate`'
				];
				$sqlFromTable = "trn_orderoutfiles A";
				$sqlWhere = $where->sql;

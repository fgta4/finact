...
/**
 * finact/procurement/orderout/apis/approval-save.php
 *
 * ==========
 * Detil-Save
 * ==========
...
$API = new class extends orderoutBase {
	...
	public function execute($data, $options, $files) {
		...
		if (class_exists($handlerclassname)) {
			$hnd = new orderout_approvalHandler($data, $options);
			$hnd->caller = &$this;
			$hnd->db = &$this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			$sqlUpdateField  = [
					'orderoutappr_id', 'orderoutappr_isapproved', 'orderoutappr_by', 'orderoutappr_date',
					'orderout_version', 'orderoutappr_isdeclined', 'orderoutappr_declinedby', 'orderoutappr_declineddate',
					'orderoutappr_notes', 'orderout_id', 'docauth_descr', 'docauth_order',
					'docauth_value', 'docauth_min', 'authlevel_id', 'authlevel_name',
					'auth_id', 'auth_name'
			];
			...
			try {
				...
				// result
				$options->criteria = [
					"orderoutappr_id" => $obj->orderoutappr_id
				];

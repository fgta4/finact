...
export function createnew(data, options) {
	...
	txt_title.html(data.orderout_id)
...
export function OpenDetil(data) {
	...
	var fn_listloaded = async (result, options) => {
		...
		setTimeout(()=>{
			var checkcolumns = document.querySelectorAll('#pnl_editapprovalgrid-tbl_list .rowcheck');
			...
			var selectbutton = document.getElementById('pnl_editapprovalgrid-tbl_list-selectall-button');
			if (detilform.AllowRemoveRecord) {
...
function grd_list_rowrender(tr) {
	...
	$(tr).find('td').each((i, td) => {
		...
		if (mapping=='docauth_descr') {
			...
			var authby = "";
			var notes = "";			
			if (record.orderoutappr_isdeclined=="1") {
				gambar = `<img src="images/xtiondecl.png" width="20px" height="20px">`;
				authby = `<div><span style="font-weight: bold">${record.orderoutappr_declinedby}</span> (${record.orderoutappr_declineddate})</div>`
				notes = record.orderoutappr_notes.replace(/(?:\r\n|\r|\n)/g, '<br>');
				notes = `<div style="margin-top: 5px; font-style: italic">${notes}</div>`
			} else if (record.orderoutappr_isapproved=="1") {
				gambar = `<img src="images/xtionappr.png" width="20px" height="20px">`;
				authby = `<div><span style="font-weight: bold">${record.orderoutappr_by}</span> (${record.orderoutappr_date})</div>`
			} else {

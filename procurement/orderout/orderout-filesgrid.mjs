...
import * as hnd from  './orderout-filesgrid-hnd.mjs'

const tbl_list = $('#pnl_editfilesgrid-tbl_list');
const txt_title = $('#pnl_editfilesgrid-title');
const pnl_control = $('.pnl_editfilesgrid-control');
const btn_removechecked  = $('#pnl_editfilesgrid-removechecked');
const btn_addrow = $('#pnl_editfilesgrid-addrow');
...
export function createnew(data, options) {
	...
	txt_title.html(data.orderout_descr)
...
export function OpenDetil(data) {
	...
	var fn_listloading = async (options) => {
		options.api = `${global.modulefullname}/files-list`
		options.criteria['id'] = data.orderout_id
	...
	var fn_listloaded = async (result, options) => {
		...
		var detilform = $ui.getPages().ITEMS['pnl_editfilesform'].handler.getForm()
		...
		setTimeout(()=>{
			var checkcolumns = document.querySelectorAll('#pnl_editfilesgrid-tbl_list .rowcheck');
			...
			var selectbutton = document.getElementById('pnl_editfilesgrid-tbl_list-selectall-button');
			if (detilform.AllowRemoveRecord) {
...
function grd_list_rowclick(tr, ev) {
	...
	$ui.getPages().show('pnl_editfilesform', () => {
		$ui.getPages().ITEMS['pnl_editfilesform'].handler.open(record, trid, header_data)
	})	
}
		...
		yes: () => {
			grd_list.removechecked({
				OnRemoving : async (options) => {
					var apiurl = `${global.modulefullname}/files-delete`
...
function btn_addrow_click() {
	$ui.getPages().show('pnl_editfilesform', ()=>{
		$ui.getPages().ITEMS['pnl_editfilesform'].handler.createnew(header_data)
	})	
}
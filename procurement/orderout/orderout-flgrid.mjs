...
const tbl_list = $('#pnl_editflgrid-tbl_list');
const txt_title = $('#pnl_editflgrid-title');
const pnl_control = $('.pnl_editflgrid-control');
const btn_removechecked  = $('#pnl_editflgrid-removechecked');
const btn_addrow = $('#pnl_editflgrid-addrow');
...
export function OpenDetil(data) {
	...
	var fn_listloading = async (options) => {
		options.api = `${global.modulefullname}/fl-list`
	...
	var fn_listloaded = async (result, options) => {
		...
		var detilform = $ui.getPages().ITEMS['pnl_editflform'].handler.getForm()
...
function grd_list_rowclick(tr, ev) {
	...
	$ui.getPages().show('pnl_editflform', () => {
		$ui.getPages().ITEMS['pnl_editflform'].handler.open(record, trid, header_data)
	})	
}
		...
		yes: () => {
			grd_list.removechecked({
				OnRemoving : async (options) => {
					var apiurl = `${global.modulefullname}/fl-delete`
...
function btn_addrow_click() {
	$ui.getPages().show('pnl_editflform', ()=>{
		$ui.getPages().ITEMS['pnl_editflform'].handler.createnew(header_data)
	})	
}
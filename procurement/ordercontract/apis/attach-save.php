...
/**
 * finact/procurement/ordercontract/apis/attach-save.php
 *
 * ==========
 * Detil-Save
 * ==========
 * Menampilkan satu baris data/record sesuai PrimaryKey,
 * dari tabel attach ordercontract (trn_ordercontract)
 *
...
$API = new class extends ordercontractBase {
	...
	public function execute($data, $options, $files) {
		$tablename = 'trn_ordercontracattch';
		$primarykey = 'ordercontracattch_id';
		...
		try {
			...
			if ($obj->ordercontracattch_descr=='') { $obj->ordercontracattch_descr = '--NULL--'; }
			...
			try {
				...
				$header_table = 'trn_ordercontract';
				$header_primarykey = 'ordercontract_id';
				...
				$fieldname = 'ordercontracattch_file';	
				if (property_exists($files, $fieldname)) {
					...
					$obj->ordercontracattch_file = $rev;
					$cmd = \FGTA4\utils\SqlUtility::CreateSQLUpdate($tablename, $obj, $key);
					$stmt = $this->db->prepare($cmd->sql);
					$stmt->execute($cmd->params);
				}				
				...
				$sql = \FGTA4\utils\SqlUtility::Select($tablename , [
					$primarykey
					, 'ordercontracattch_id', 'ordercontracattch_name', 'ordercontracattch_descr', 'ordercontracattch_order', 'ordercontracattch_file', 'ordercontract_id', '_createby', '_createdate', '_modifyby', '_modifydate', '_createby', '_createdate', '_modifyby', '_modifydate'
				], $where->sql);

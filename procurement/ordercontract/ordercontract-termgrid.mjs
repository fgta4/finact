...
const tbl_list = $('#pnl_edittermgrid-tbl_list');
const txt_title = $('#pnl_edittermgrid-title');
const pnl_control = $('#pnl_edittermgrid-control');
const btn_removechecked  = $('#pnl_edittermgrid-removechecked');
const btn_addrow = $('#pnl_edittermgrid-addrow');
...
export function OpenDetil(data) {
	...
	var fn_listloading = async (options) => {
		options.api = `${global.modulefullname}/term-list`
	...
	var fn_listloaded = async (result, options) => {
		...
		var detilform = $ui.getPages().ITEMS['pnl_edittermform'].handler.getForm()
...
function grd_list_rowclick(tr, ev) {
	...
	$ui.getPages().show('pnl_edittermform', () => {
		$ui.getPages().ITEMS['pnl_edittermform'].handler.open(record, trid, header_data)
	})	
}
		...
		yes: () => {
			grd_list.removechecked({
				OnRemoving : async (options) => {
					var apiurl = `${global.modulefullname}/term-delete`
...
function btn_addrow_click() {
	$ui.getPages().show('pnl_edittermform', ()=>{
		$ui.getPages().ITEMS['pnl_edittermform'].handler.createnew(header_data)
	})	
}
...
import * as hnd from  './inquirytype-itemclassgrid-hnd.mjs'

const tbl_list = $('#pnl_edititemclassgrid-tbl_list');
const txt_title = $('#pnl_edititemclassgrid-title');
const pnl_control = $('.pnl_edititemclassgrid-control');
const btn_removechecked  = $('#pnl_edititemclassgrid-removechecked');
const btn_addrow = $('#pnl_edititemclassgrid-addrow');
...
export function OpenDetil(data) {
	...
	var fn_listloading = async (options) => {
		options.api = `${global.modulefullname}/itemclass-list`
	...
	var fn_listloaded = async (result, options) => {
		...
		var detilform = $ui.getPages().ITEMS['pnl_edititemclassform'].handler.getForm()
...
function grd_list_rowclick(tr, ev) {
	...
	$ui.getPages().show('pnl_edititemclassform', () => {
		$ui.getPages().ITEMS['pnl_edititemclassform'].handler.open(record, trid, header_data)
	})	
}
		...
		yes: () => {
			grd_list.removechecked({
				OnRemoving : async (options) => {
					var apiurl = `${global.modulefullname}/itemclass-delete`
...
function btn_addrow_click() {
	$ui.getPages().show('pnl_edititemclassform', ()=>{
		$ui.getPages().ITEMS['pnl_edititemclassform'].handler.createnew(header_data)
	})	
}
...
if (is_file(__DIR__ .'/data-itemclass-handler.php')) {
	require_once __DIR__ .'/data-itemclass-handler.php';
}
...
/**
 * finact/procurement/inquirytype/apis/itemclass-save.php
 *
 * ==========
 * Detil-Save
 * ==========
 * Menampilkan satu baris data/record sesuai PrimaryKey,
 * dari tabel itemclass inquirytype (mst_inquirytypeitemclass)
 *
...
$API = new class extends inquirytypeBase {
	...
	public function execute($data, $options) {
		$event = 'on-save';
		$tablename = 'mst_inquirytypeitemclass';
		$primarykey = 'inquirytypeitemclass_id';
		$autoid = $options->autoid;
		$datastate = $data->_state;

		$userdata = $this->auth->session_get_user();

		$handlerclassname = "\\FGTA4\\apis\\inquirytype_itemclassHandler";
		if (class_exists($handlerclassname)) {
			$hnd = new inquirytype_itemclassHandler($data, $options);
			$hnd->caller = &$this;
			$hnd->db = &$this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			try {
				...
				// result
				$options->criteria = [
					"inquirytypeitemclass_id" => $obj->inquirytypeitemclass_id
				];

				$criteriaValues = [
					"inquirytypeitemclass_id" => " inquirytypeitemclass_id = :inquirytypeitemclass_id "
				];
				...
				$sqlFieldList = [
					'inquirytypeitemclass_id' => 'A.`inquirytypeitemclass_id`', 'itemclass_id' => 'A.`itemclass_id`', 'inquirytype_id' => 'A.`inquirytype_id`', '_createby' => 'A.`_createby`',
					'_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`', '_modifyby' => 'A.`_modifyby`', '_modifydate' => 'A.`_modifydate`'
				];
				$sqlFromTable = "mst_inquirytypeitemclass A";
				$sqlWhere = $where->sql;

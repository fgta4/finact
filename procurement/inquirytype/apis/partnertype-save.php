...
if (is_file(__DIR__ .'/data-partnertype-handler.php')) {
	require_once __DIR__ .'/data-partnertype-handler.php';
}
...
/**
 * finact/procurement/inquirytype/apis/partnertype-save.php
 *
 * ==========
 * Detil-Save
 * ==========
 * Menampilkan satu baris data/record sesuai PrimaryKey,
 * dari tabel partnertype inquirytype (mst_inquirytypepartnertype)
 *
...
$API = new class extends inquirytypeBase {
	...
	public function execute($data, $options) {
		$event = 'on-save';
		$tablename = 'mst_inquirytypepartnertype';
		$primarykey = 'inquirytypepartnertype_id';
		$autoid = $options->autoid;
		$datastate = $data->_state;

		$userdata = $this->auth->session_get_user();

		$handlerclassname = "\\FGTA4\\apis\\inquirytype_partnertypeHandler";
		if (class_exists($handlerclassname)) {
			$hnd = new inquirytype_partnertypeHandler($data, $options);
			$hnd->caller = &$this;
			$hnd->db = &$this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			$obj->inquirytype_id = strtoupper($obj->inquirytype_id);
			...
			try {
				...
				$header_table = 'mst_inquirytype';
				$header_primarykey = 'inquirytype_id';
				...
				// result
				$options->criteria = [
					"inquirytypepartnertype_id" => $obj->inquirytypepartnertype_id
				];

				$criteriaValues = [
					"inquirytypepartnertype_id" => " inquirytypepartnertype_id = :inquirytypepartnertype_id "
				];
				...
				$sqlFieldList = [
					'inquirytypepartnertype_id' => 'A.`inquirytypepartnertype_id`', 'partnertype_id' => 'A.`partnertype_id`', 'inquirytype_id' => 'A.`inquirytype_id`', '_createby' => 'A.`_createby`',
					'_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`', '_modifyby' => 'A.`_modifyby`', '_modifydate' => 'A.`_modifydate`'
				];
				$sqlFromTable = "mst_inquirytypepartnertype A";
				$sqlWhere = $where->sql;

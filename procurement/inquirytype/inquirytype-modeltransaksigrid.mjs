...
const tbl_list = $('#pnl_editmodeltransaksigrid-tbl_list');
const txt_title = $('#pnl_editmodeltransaksigrid-title');
const pnl_control = $('.pnl_editmodeltransaksigrid-control');
const btn_removechecked  = $('#pnl_editmodeltransaksigrid-removechecked');
const btn_addrow = $('#pnl_editmodeltransaksigrid-addrow');
...
export function createnew(data, options) {
	...
	grd_list.clear();
	txt_title.html(data.inquirytype_name)
	header_data = data;
}
...
export function OpenDetil(data) {
	...
	var fn_listloading = async (options) => {
		options.api = `${global.modulefullname}/modeltransaksi-list`
		options.criteria['id'] = data.inquirytype_id
	...
	var fn_listloaded = async (result, options) => {
		...
		var detilform = $ui.getPages().ITEMS['pnl_editmodeltransaksiform'].handler.getForm()
...
function grd_list_rowclick(tr, ev) {
	...
	$ui.getPages().show('pnl_editmodeltransaksiform', () => {
		$ui.getPages().ITEMS['pnl_editmodeltransaksiform'].handler.open(record, trid, header_data)
	})	
}
		...
		yes: () => {
			grd_list.removechecked({
				OnRemoving : async (options) => {
					var apiurl = `${global.modulefullname}/modeltransaksi-delete`
...
function btn_addrow_click() {
	$ui.getPages().show('pnl_editmodeltransaksiform', ()=>{
		$ui.getPages().ITEMS['pnl_editmodeltransaksiform'].handler.createnew(header_data)
	})	
}
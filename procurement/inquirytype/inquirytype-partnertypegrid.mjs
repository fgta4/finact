...
const tbl_list = $('#pnl_editpartnertypegrid-tbl_list');
const txt_title = $('#pnl_editpartnertypegrid-title');
const pnl_control = $('.pnl_editpartnertypegrid-control');
const btn_removechecked  = $('#pnl_editpartnertypegrid-removechecked');
const btn_addrow = $('#pnl_editpartnertypegrid-addrow');
...
export function OpenDetil(data) {
	...
	var fn_listloading = async (options) => {
		options.api = `${global.modulefullname}/partnertype-list`
	...
	var fn_listloaded = async (result, options) => {
		...
		var detilform = $ui.getPages().ITEMS['pnl_editpartnertypeform'].handler.getForm()
...
function grd_list_rowclick(tr, ev) {
	...
	$ui.getPages().show('pnl_editpartnertypeform', () => {
		$ui.getPages().ITEMS['pnl_editpartnertypeform'].handler.open(record, trid, header_data)
	})	
}
		...
		yes: () => {
			grd_list.removechecked({
				OnRemoving : async (options) => {
					var apiurl = `${global.modulefullname}/partnertype-delete`
...
function btn_addrow_click() {
	$ui.getPages().show('pnl_editpartnertypeform', ()=>{
		$ui.getPages().ITEMS['pnl_editpartnertypeform'].handler.createnew(header_data)
	})	
}
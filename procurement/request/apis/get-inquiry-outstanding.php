...
$API = new class extends requestBase {
	...
	public function execute($options) {
		...
		try {
			...
			\FGTA4\utils\SqlUtility::setDefaultCriteria($options->criteria, 'inquiry_outstanding', '1');
			$where = \FGTA4\utils\SqlUtility::BuildCriteria(
				$options->criteria,
				[
					"search" => " A.inquiry_id LIKE CONCAT('%', :search, '%') OR A.inquiry_descr LIKE CONCAT('%', :search, '%') ",
					"user_dept_id" => " A.user_dept_id = :user_dept_id ",
					"owner_dept_id" => " A.owner_dept_id = :owner_dept_id ",
					"inquiry_istoberequest" => " A.inquiry_istoberequest = '1' ",
					"inquiry_outstanding" => " (A.inquiry_isapproved=1 AND A.inquiry_iscomplete=0 AND A.inquiry_isclose=0 ) "
				]
			);
			...
			$stmt = $this->db->prepare("
				select 
				  A.inquiry_id, A.inquirytype_id, A.inquiry_descr, A.inquiry_dtstart, A.inquiry_dtend, A.user_dept_id, A.empl_id
				, A.inquiry_isadvance
				, A.project_id, A.projecttask_id, A.projbudget_id, A.projbudgettask_id, A.inquiry_isunbudgetted, A.site_id, A.deliver_siteaddress
				, A.deliver_city, A.deliver_upname, A.deliver_uptelp, A.inquirymodel_id, A.inquiryselect_id, A.inquiry_selectfield, A.owner_dept_id, A.request_dept_id,  A.request_doc_id
				, A.orderout_dept_id, A.orderout_doc_id, A.trxmodel_id, A.itemmanage_id, A.inquiry_rejectnotes, A.doc_id, A.inquiry_version
				, A.inquiry_maxadvancevalue, A.inquiry_isallowadvance, A.inquiry_isdateinterval, A.inquiry_iscommit, A.inquiry_commitby, A.inquiry_commitdate
				, A.inquiry_isapprovalprogress, A.inquiry_isapproved, A.inquiry_approveby, A.inquiry_approvedate, A.inquiry_isdeclined, A.inquiry_declineby
				, A.inquiry_declinedate, A.inquiry_ispreparing, A.inquiry_isprepared, A.inquiry_preparedby, A.inquiry_prepareddate, A.inquiry_isreject
				, A.inquiry_rejectby, A.inquiry_rejectdate, A.inquiry_iscomplete, A.inquiry_isclose, A.inquiry_closeby, A.inquiry_closedate
				, A.inquiry_isautogenerated, A.inquiry_isitemdeptuser, A.inquiry_isitemdeptowner, A.inquiry_istoberequest

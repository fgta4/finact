...
$API = new class extends inquiryprocessBase {
	...
	public function execute($options) {
		...
		try {
			...
			$stmt = $this->db->prepare("select count(*) as n from trn_inquiryitem A" . $where->sql);
			...
			foreach ($rows as $row) {
				...
				array_push($records, array_merge($record, [
					...
					'inquirydetil_descr' => \FGTA4\utils\SqlUtility::Lookup($record['inquirydetil_id'], $this->db, 'trn_inquirydetil', 'inquirydetil_id', 'inquirydetil_descr'),

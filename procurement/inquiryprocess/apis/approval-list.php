...
/**
 * finact/procurement/inquiryprocess/apis/approval-list.php
 *
 * ==============
 * Detil-DataList
 * ==============
 * Menampilkan data-data pada tabel approval inquiryprocess (trn_inquiry)
...
$API = new class extends inquiryprocessBase {
	...
	public function execute($options) {
		...
		$handlerclassname = "\\FGTA4\\apis\\inquiryprocess_approvalHandler";
		if (class_exists($handlerclassname)) {
			$hnd = new inquiryprocess_approvalHandler($data, $options);
			$hnd->caller = $this;
			$hnd->db = $this->db;
			$hnd->auth = $this->auth;
